{
  fafafa.ssl.abstract.types - SSL/TLS 抽象类型定义
  ...
  描述:
    定义 fafafa.ssl 库的所有抽象类型、常量、枚举和异常类。
    这些类型完全独立于任何特定的 SSL/TLS 后端实现（OpenSSL, WinSSL 等）。
    所有后端实现必须使用这些类型以保证 API 的一致性。
    
  架构位置:
    抽象层 - 不依赖于任何具体后端实现
}

unit fafafa.ssl.abstract.types;

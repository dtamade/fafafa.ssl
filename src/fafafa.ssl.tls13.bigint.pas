...
type
  TBigNat = array of UInt16;  // little-endian limbs
...
function BigNatFromWord(AValue: UInt16): TBigNat;
begin
  if AValue = 0 then
  begin
    SetLength(Result, 0);
...
begin
  ...
  begin
    ...
    Result[I] := UInt16(LDiff and LIMB_MASK);
  end;
...
begin
  ...
  begin
    ...
    LSum := LA + LB + LCarry;
    Result[I] := UInt16(LSum and LIMB_MASK);
  ...
  begin
    ...
    Result[LLen] := UInt16(LCarry and LIMB_MASK);
  end;

  NormalizeBigNat(Result);
end;

procedure BigNatAddWord(var AValue: TBigNat; AWord: UInt16);
var
  I: Integer;
  LCarry: UInt32;
  LSum: UInt32;
...
begin
  ...
  FillChar(Result[0], Length(Result) * SizeOf(UInt16), 0);
  ...
  begin
    ...
    begin
      ...
      Result[I + J] := UInt16(LUV and LIMB_MASK);
    ...
    begin
      ...
      Result[K] := UInt16(LUV and LIMB_MASK);
      LCarry := LUV shr LIMB_BITS;
      Inc(K);
    end;
  end;
...
begin
  ...
  LLimbCount := (Length(ABytes) + 1) div 2;
  SetLength(Result, LLimbCount);
  FillChar(Result[0], LLimbCount * SizeOf(UInt16), 0);
  ...
  begin
    ...
    begin
      Result[LLimbIndex] := UInt16(LWord and LIMB_MASK);
      Inc(LLimbIndex);
      LWord := 0;
      LShift := 0;
    end;
...
begin
  ...
  begin
    LLimbIndex := LByteIndex div 2;
    LShift := (LByteIndex mod 2) * 8;
...
function TryComputeMontgomeryNPrime(AN0: UInt16; out ANPrime: UInt16): Boolean;
var
  LInv: UInt32;
  LFactor: UInt32;
  LTmp: UInt32;
...
begin
  ...
  for I := 0 to 3 do
  begin
    LFactor := UInt32((UInt64(AN0) * UInt64(LInv)) and LIMB_MASK);
    LTmp := UInt32((UInt64(LIMB_BASE) + 2 - UInt64(LFactor)) and LIMB_MASK);
    LInv := UInt32((UInt64(LInv) * UInt64(LTmp)) and LIMB_MASK);
  end;

  ANPrime := UInt16((LIMB_BASE - LInv) and LIMB_MASK);
  Result := True;
end;

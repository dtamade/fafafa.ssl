{**
 * Unit: fafafa.ssl.tls13.primitives
 * Purpose: TLS 1.3 所需基础密码学原语（纯 Pascal）
 *
 * 当前提供：HMAC-SHA256、HKDF-Extract/Expand、TLS 1.3 HKDF-Expand-Label
 *}
...
begin
  ...
  SetLength(LHkdfLabel, 0);
  AppendUInt16(LHkdfLabel, Word(ALength));
  AppendByte(LHkdfLabel, Byte(LLabelLen));
  AppendBytes(LHkdfLabel, LLabelBytes);
  AppendByte(LHkdfLabel, Byte(LContextLen));
  AppendBytes(LHkdfLabel, AContext);

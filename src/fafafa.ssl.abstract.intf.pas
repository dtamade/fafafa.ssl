{
  fafafa.ssl.abstract.intf - SSL/TLS 抽象接口定义
  ...
  描述:
    ...
    这些接口完全独立于任何特定的 SSL/TLS 后端实现（OpenSSL, WinSSL 等）。
    所有后端实现必须实现这些接口以保证 API 的一致性。
...
uses
  SysUtils, Classes, fafafa.ssl.abstract.types;

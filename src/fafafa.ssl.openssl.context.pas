...
begin
  ...
  begin
    if sslProtocolSSL2 in LDeprecated then
      TSecurityLog.Warning('OpenSSL', 'SSL 2.0 已废弃且不安全，强烈建议禁用');
    if sslProtocolSSL3 in LDeprecated then
      TSecurityLog.Warning('OpenSSL', 'SSL 3.0 已废弃（POODLE漏洞），强烈建议禁用');
    if sslProtocolTLS10 in LDeprecated then
      TSecurityLog.Warning('OpenSSL', 'TLS 1.0 已废弃，建议升级到 TLS 1.2 或更高版本');
    if sslProtocolTLS11 in LDeprecated then
      TSecurityLog.Warning('OpenSSL', 'TLS 1.1 已废弃，建议升级到 TLS 1.2 或更高版本');

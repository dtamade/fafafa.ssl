...
begin
  ...
  LAccumulator := P256InfinityPoint;

  for LByteIndex := 0 to Length(LScalar32) - 1 do
  begin
    for LBitIndex := 7 downto 0 do
    begin
      if not TryP256PointDouble(LAccumulator, LDoubled, AError) then
        Exit;
      LAccumulator := LDoubled;

      LBit := (LScalar32[LByteIndex] shr LBitIndex) and 1;
      if LBit = 1 then
      begin
        if not TryP256PointAdd(LAccumulator, APoint, LAdded, AError) then
          Exit;
        LAccumulator := LAdded;
      end;
    end;

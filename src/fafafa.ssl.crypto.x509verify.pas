unit fafafa.ssl.crypto.x509verify;
...
uses
  SysUtils, Classes, fafafa.ssl.x509;
...
begin
  ...
  begin
    ...
    if not VerifyRSASignature(LCurrent, LIssuer) then
    begin
  ...
  begin
    if not VerifyRSASignature(AChain[I], AChain[I + 1]) then
    begin
      Result.ErrorCode := X509_V_ERR_CERT_SIGNATURE_FAILURE;

program test_connector_timeout_safety_entry;
...
procedure AssertEquals(const AExpected, AActual: Integer; ACode: Integer);
begin
...
begin
  Result := TMockClientConnection.Create(Self);
end;
...
begin
  ...
  StreamTransport := TMemoryStream.Create;
  TLS := nil;
  try

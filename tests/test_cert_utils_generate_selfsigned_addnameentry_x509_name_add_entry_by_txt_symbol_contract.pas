program test_cert_utils_generate_selfsigned_addnameentry_x509_name_add_entry_by_txt_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-addnameentry-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenX509NameAddEntryByTxtIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalX509NameAddEntryByTxt: TX509_NAME_add_entry_by_txt;
begin
  WriteLn;
  WriteLn('=== Certificate utils self-signed AddNameEntry X509_NAME_add_entry_by_txt symbol guard ===');

  if not Assigned(X509_NAME_add_entry_by_txt) then
  begin
    MarkSkip('certificate utils selfsigned AddNameEntry X509_NAME_add_entry_by_txt contract',
      'required baseline X509_NAME_add_entry_by_txt helper is unavailable');
    Exit;
  end;
  ...
  LOriginalX509NameAddEntryByTxt := X509_NAME_add_entry_by_txt;
  X509_NAME_add_entry_by_txt := nil;
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when X509_NAME_add_entry_by_txt is unavailable',
      LOptions
    );
  finally
    X509_NAME_add_entry_by_txt := LOriginalX509NameAddEntryByTxt;
  end;
end;

begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned AddNameEntry X509_NAME_add_entry_by_txt Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenX509NameAddEntryByTxtIsUnavailable;

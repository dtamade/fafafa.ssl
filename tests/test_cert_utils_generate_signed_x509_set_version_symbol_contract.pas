program test_cert_utils_generate_signed_x509_set_version_symbol_contract;
...
begin
  ...
  Result.CommonName := 'signed-x509-set-version-root.local';
...
begin
  Result := TCertificateUtils.DefaultGenOptions;
  Result.CommonName := 'signed-x509-set-version-leaf.local';
  Result.Organization := 'fafafa.ssl contract';
  Result.IsCA := False;
  Result.ValidDays := 30;
end;
...
procedure TestGenerateSignedShouldFailGracefullyWhenX509SetVersionIsUnavailable;
var
  LLeafOptions: TCertGenOptions;
  LCACertPEM: string;
  LCAKeyPEM: string;
  LOriginalX509SetVersion: TX509_set_version;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSigned X509_set_version symbol guard ===');

  if not Assigned(X509_set_version) then
  begin
    MarkSkip('certificate utils generate-signed X509_set_version contract',
      'required baseline X509_set_version helper is unavailable');
    Exit;
  end;
  ...
  LOriginalX509SetVersion := X509_set_version;
  X509_set_version := nil;
  try
    AssertGenerateSignedControlledFailure(
      'GenerateSigned when X509_set_version is unavailable',
      LLeafOptions,
      LCACertPEM,
      LCAKeyPEM
    );
  finally
    X509_set_version := LOriginalX509SetVersion;
  end;
end;

begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSigned X509_set_version Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSignedShouldFailGracefullyWhenX509SetVersionIsUnavailable;

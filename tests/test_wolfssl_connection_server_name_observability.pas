program test_wolfssl_connection_server_name_observability;
...
begin
  WriteLn('fafafa.ssl - WolfSSL ServerName should be observable via connection info');
  ...
  try
    ...
    Require(LConnection <> nil, 'WolfSSL connection should not be nil');
    Require(Supports(LConnection, ISSLClientConnection, LClientConn),
      'WolfSSL connection should support ISSLClientConnection');
  ...
  WriteLn('[PASS] WolfSSL connection info exposes effective ServerName');
end.

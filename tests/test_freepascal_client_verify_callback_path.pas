program test_freepascal_client_verify_callback_path;
...
type
  ...
  public
    ...
    function HandleVerify(const ACertificate: TSSLCertificateInfo;
...
constructor TVerifyCallbackProbe.Create(AResult: Boolean);
begin
  inherited Create;
  FResult := AResult;
  FCallCount := 0;
...
function TVerifyCallbackProbe.HandleVerify(const ACertificate: TSSLCertificateInfo;
  const AErrorCode: Integer; const AErrorMessage: string): Boolean;
begin
  Inc(FCallCount);
...
procedure TestVerifyCallbackOnClientHandshakePath;
var
  ...
  LVetoProbe: TVerifyCallbackProbe;
  LOverrideProbe: TVerifyCallbackProbe;
begin
  ...
  LVetoProbe := TVerifyCallbackProbe.Create(False);
  try
    LClientCtx := TSSLFactory.CreateContext(sslCtxClient, sslFreePascal);
    AssertTrue(LClientCtx <> nil, 'FreePascal client context should be created for veto case');
    LClientCtx.SetPreferredVersion(sslProtocolTLS13);
    LClientCtx.SetCertificateStore(CreateTrustedStore(LServerCertPEM));
    LClientCtx.SetVerifyCallback(@LVetoProbe.HandleVerify);
    ...
    try
      ...
      AssertTrue(LClientConn <> nil, 'Client connection should be created for veto case');
      RequireServerName(LClientConn, 'alt.example.com');
      AssertTrue(not LClientConn.Connect,
        'Verify callback returning False should veto successful verification');
      AssertTrue(LVetoProbe.CallCount > 0,
        'Verify callback should be invoked on successful built-in verification path');
    finally
  ...
  LOverrideProbe := TVerifyCallbackProbe.Create(True);
  try
    LClientCtx := TSSLFactory.CreateContext(sslCtxClient, sslFreePascal);
    AssertTrue(LClientCtx <> nil, 'FreePascal client context should be created for override case');
    LClientCtx.SetPreferredVersion(sslProtocolTLS13);
    LClientCtx.SetCertificateStore(CreateTrustedStore(LServerCertPEM));
    LClientCtx.SetVerifyCallback(@LOverrideProbe.HandleVerify);

    LClientTransport := TServerResponderStream.Create(LServerCertPEM, LServerKeyPEM);
    try
      LClientConn := LClientCtx.CreateConnection(LClientTransport);
      AssertTrue(LClientConn <> nil, 'Client connection should be created for override case');
      RequireServerName(LClientConn, 'wrong.example.com');
      AssertTrue(LClientConn.Connect,
        'Verify callback returning True should be able to override verification failure');
      AssertTrue(LOverrideProbe.CallCount > 0,
        'Verify callback should be invoked on failure path');
      AssertTrue(Pos('hostname', LowerCase(LOverrideProbe.LastErrorMessage)) > 0,
        'Override path should observe hostname failure message');
    finally
      LClientTransport.Free;
    end;
  finally
...
begin
  WriteLn('Testing FreePascal client verify callback path...');
  TestVerifyCallbackOnClientHandshakePath;
  WriteLn('✅ FreePascal client verify callback path checks passed');
end.

...
对于 session-resumption lane，这些 marker 现在还要这样读：
...
这样才方便后续直接在 artifact 里区分 shared/public conservative truth 与 opt-in native probe evidence，而不是把 `observed_reuse` 单独误读成“是否真的观测到 resumed handshake”的唯一结论。

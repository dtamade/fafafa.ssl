program test_cert_utils_generate_signed_asn1_integer_set_symbol_contract;
...
begin
  ...
  Result.CommonName := 'signed-asn1-integer-set-root.local';
...
begin
  Result := TCertificateUtils.DefaultGenOptions;
  Result.CommonName := 'signed-asn1-integer-set-leaf.local';
  Result.Organization := 'fafafa.ssl contract';
  Result.IsCA := False;
  Result.ValidDays := 30;
end;
...
procedure TestGenerateSignedShouldFailGracefullyWhenASN1IntegerSetIsUnavailable;
var
  LLeafOptions: TCertGenOptions;
  LCACertPEM: string;
  LCAKeyPEM: string;
  LOriginalASN1IntegerSet: TASN1_INTEGER_set;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSigned ASN1_INTEGER_set symbol guard ===');

  if not Assigned(ASN1_INTEGER_set) then
  begin
    MarkSkip('certificate utils generate-signed ASN1_INTEGER_set contract',
      'required baseline ASN1_INTEGER_set helper is unavailable');
    Exit;
  end;
  ...
  LOriginalASN1IntegerSet := ASN1_INTEGER_set;
  ASN1_INTEGER_set := nil;
  try
    AssertGenerateSignedControlledFailure(
      'GenerateSigned when ASN1_INTEGER_set is unavailable',
      LLeafOptions,
      LCACertPEM,
      LCAKeyPEM
    );
  finally
    ASN1_INTEGER_set := LOriginalASN1IntegerSet;
  end;
end;

begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSigned ASN1_INTEGER_set Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    begin
      ...
      if not LoadOpenSSLASN1(GetCryptoLibHandle) then
        raise Exception.Create('failed to load ASN1 support');
    end;

    if SkippedTests = 0 then
      TestGenerateSignedShouldFailGracefullyWhenASN1IntegerSetIsUnavailable;

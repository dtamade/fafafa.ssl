program test_cert_utils_generate_selfsigned_x509_get_subject_name_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-x509-get-subject-name-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenX509GetSubjectNameIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalX509GetSubjectName: TX509_get_subject_name;
begin
  WriteLn;
  WriteLn('=== Certificate utils self-signed X509_get_subject_name symbol guard ===');

  if not Assigned(X509_get_subject_name) then
  begin
    MarkSkip('certificate utils selfsigned X509_get_subject_name contract',
  ...
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when X509_get_subject_name is unavailable',
      LOptions
    );
  finally
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned X509_get_subject_name Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenX509GetSubjectNameIsUnavailable;

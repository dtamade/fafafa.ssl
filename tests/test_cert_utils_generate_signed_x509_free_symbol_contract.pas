program test_cert_utils_generate_signed_x509_free_symbol_contract;
...
begin
  ...
  Result.CommonName := 'signed-x509-free-root.local';
...
begin
  Result := TCertificateUtils.DefaultGenOptions;
  Result.CommonName := 'signed-x509-free-leaf.local';
  Result.Organization := 'fafafa.ssl contract';
  Result.IsCA := False;
  Result.ValidDays := 30;
end;
...
procedure RestoreX509FreeDuringUnwind(APKey: PEVP_PKEY); cdecl;
begin
  Inc(GEVPPKeyFreeCallCount);
  if GEVPPKeyFreeCallCount = 1 then
    X509_free := GOriginalX509Free;
  GOriginalEVPPKeyFree(APKey);
end;

procedure InstallX509FreeFailureWrapper;
begin
  GEVPPKeyFreeCallCount := 0;
  X509_free := nil;
  EVP_PKEY_free := @RestoreX509FreeDuringUnwind;
end;
...
procedure TestGenerateSignedShouldFailGracefullyWhenX509FreeIsUnavailable;
var
  LLeafOptions: TCertGenOptions;
  LCACertPEM: string;
  LCAKeyPEM: string;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSigned X509_free symbol guard ===');

  if (not Assigned(X509_free)) or (not Assigned(EVP_PKEY_free)) then
  begin
    MarkSkip('certificate utils generate-signed X509_free contract',
      'required baseline X509_free/EVP_PKEY_free helpers are unavailable');
    Exit;
  end;
  ...
  try
    ...
    AssertGenerateSignedControlledFailure(
      'GenerateSigned when X509_free is unavailable',
      LLeafOptions,
      LCACertPEM,
      LCAKeyPEM
    );
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSigned X509_free Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSignedShouldFailGracefullyWhenX509FreeIsUnavailable;

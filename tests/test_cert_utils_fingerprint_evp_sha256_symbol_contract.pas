program test_cert_utils_fingerprint_evp_sha256_symbol_contract;
...
procedure TestGetFingerprintShouldFailControlledWhenEVPSHA256IsUnavailable;
var
  LFixturePEM: string;
...
begin
  WriteLn;
  WriteLn('=== Certificate utils fingerprint EVP_sha256 symbol guard ===');
  ...
  begin
    MarkSkip('certificate utils fingerprint EVP_sha256 symbol contract',
  ...
  try
    ...
    AssertTrue('GetFingerprint when EVP_sha256 is unavailable should raise',
      LRaised, 'expected GetFingerprint to fail');
    AssertTrue('GetFingerprint when EVP_sha256 is unavailable should raise controlled ESSLCertError',
      LControlled, LDetail);
    AssertTrue('TryGetFingerprint when EVP_sha256 is unavailable should not raise',
      not LTryRaised, LTryDetail);
    AssertTrue('TryGetFingerprint when EVP_sha256 is unavailable should return False',
      not LTryResult,
      'expected TryGetFingerprint to return False');
    AssertTrue('TryGetFingerprint when EVP_sha256 is unavailable should clear output',
      LTryFingerprint = '',
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils Fingerprint EVP_sha256 Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGetFingerprintShouldFailControlledWhenEVPSHA256IsUnavailable;

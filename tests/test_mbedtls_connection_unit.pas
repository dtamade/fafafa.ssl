{**
 * Test: MbedTLS Connection Unit Tests
 * Purpose: High-coverage unit tests for TMbedTLSConnection
 *
 * Test Categories:
...
program test_mbedtls_connection_unit;
...
begin
  ...
  try
    ...
    on E: Exception do
      Test('CreateConnection with invalid socket', True);  // May throw, that's ok
  end;
...
begin
  ...
  try
    ...
    // Test DoHandshake on invalid socket (should fail gracefully)
    try
      LConn.DoHandshake;
...
begin
  ...
  try
    ...
    // Test GetError returns valid error code
    try
      Test('GetError returns TSSLErrorCode', True);
      LConn.GetError(MBEDTLS_ERR_SSL_WANT_READ);
    except
      Test('GetError returns TSSLErrorCode', False);
    end;
...
begin
  ...
  try
    ...
    // Test GetSelectedALPNProtocol (available in interface)
    try
...
begin
  WriteLn('MbedTLS Connection Unit Tests');
  WriteLn('==============================');
  WriteLn('High-coverage tests for TMbedTLSConnection');
  WriteLn('');
  ...
  WriteLn('');
  WriteLn('========================================');
  WriteLn('MbedTLS Connection Unit Test Summary');
  WriteLn('========================================');

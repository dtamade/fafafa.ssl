program test_cert_utils_generate_selfsigned_private_key_pem_export_pem_write_bio_privatekey_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-private-key-export-pem-write-contract.local';
...
function DisablePrivateKeyPEMWriteAfterPrivateKeyExportBIOConstructor(const AType: PBIO_METHOD): PBIO; cdecl;
begin
...
begin
  ...
  BIO_new := @DisablePrivateKeyPEMWriteAfterPrivateKeyExportBIOConstructor;
end;
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenPrivateKeyExportPEMWriteBecomesUnavailable;
var
  LOptions: TCertGenOptions;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSelfSigned private-key PEM export PEM_write_bio_PrivateKey symbol guard ===');

  if (not Assigned(BIO_new)) or (not Assigned(PEM_write_bio_PrivateKey)) then
  begin
    MarkSkip('certificate utils GenerateSelfSigned private-key PEM export PEM_write_bio_PrivateKey contract',
  ...
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when private-key PEM export PEM_write_bio_PrivateKey becomes unavailable',
      LOptions
    );
  finally
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned Private-Key PEM Export PEM_write_bio_PrivateKey Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenPrivateKeyExportPEMWriteBecomesUnavailable;

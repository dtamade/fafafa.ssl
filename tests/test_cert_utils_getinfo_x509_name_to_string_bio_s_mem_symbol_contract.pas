program test_cert_utils_getinfo_x509_name_to_string_bio_s_mem_symbol_contract;
...
procedure AssertNameConversionBIOGuardInfo(
  const AName: string;
...
begin
  ...
  begin
    try
      AssertNameConversionBIOGuardInfo(
        AName,
        LInfo,
        AExpectedVersion,
        AExpectedNotBefore,
...
procedure TestGetInfoShouldFailGracefullyWhenX509NameToStringBIOSMemIsUnavailable;
var
  LFixturePEM: string;
...
begin
  WriteLn;
  WriteLn('=== Certificate utils GetInfo X509NameToString BIO_s_mem symbol guard ===');
  ...
  begin
    MarkSkip('certificate utils getinfo X509NameToString BIO_s_mem symbol contract',
  ...
  try
    ...
    try
      AssertGetInfoSafeDegrade(
        'GetInfo when X509NameToString BIO_s_mem is unavailable',
        LFixturePEM,
        LExpectedVersion,
        LExpectedNotBefore,
        LExpectedNotAfter,
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GetInfo X509NameToString BIO_s_mem Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGetInfoShouldFailGracefullyWhenX509NameToStringBIOSMemIsUnavailable;

program test_cert_utils_generate_signed_ca_certificate_pem_read_bio_x509_symbol_contract;
...
begin
  ...
  Result.CommonName := 'signed-ca-certificate-pem-read-x509-root.local';
...
begin
  Result := TCertificateUtils.DefaultGenOptions;
  Result.CommonName := 'signed-ca-certificate-pem-read-x509-leaf.local';
  Result.Organization := 'fafafa.ssl contract';
  Result.IsCA := False;
  Result.ValidDays := 30;
end;
...
function DisableCACertificatePEMReadX509AfterBIOConstructor(const ABuf: Pointer; ALen: Integer): PBIO; cdecl;
begin
  Inc(GBIONewMemBufCallCount);
  Result := GOriginalBIONewMemBuf(ABuf, ALen);
  if (GBIONewMemBufCallCount = 1) and (Result <> nil) then
    PEM_read_bio_X509 := nil;
end;

procedure InstallCACertificatePEMReadX509FailureWrapper;
begin
  GBIONewMemBufCallCount := 0;
  PEM_read_bio_X509 := GOriginalPEMReadX509;
  BIO_new_mem_buf := @DisableCACertificatePEMReadX509AfterBIOConstructor;
end;
...
var
  ...
  LTryResult: Boolean;
begin
  InstallCACertificatePEMReadX509FailureWrapper;
...
procedure TestGenerateSignedShouldFailGracefullyWhenCACertificatePEMReadX509BecomesUnavailable;
var
  LLeafOptions: TCertGenOptions;
  LCACertPEM: string;
  LCAKeyPEM: string;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSigned CA certificate PEM_read_bio_X509 symbol guard ===');

  if (not Assigned(BIO_new_mem_buf)) or (not Assigned(PEM_read_bio_X509)) then
  begin
    MarkSkip('certificate utils generate-signed CA certificate PEM_read_bio_X509 contract',
      'required baseline BIO_new_mem_buf/PEM_read_bio_X509 helpers are unavailable');
    Exit;
  end;
  ...
  try
    AssertGenerateSignedControlledFailure(
      'GenerateSigned when CA certificate PEM_read_bio_X509 becomes unavailable',
      LLeafOptions,
      LCACertPEM,
      LCAKeyPEM
    );
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSigned CA Certificate PEM_read_bio_X509 Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSignedShouldFailGracefullyWhenCACertificatePEMReadX509BecomesUnavailable;

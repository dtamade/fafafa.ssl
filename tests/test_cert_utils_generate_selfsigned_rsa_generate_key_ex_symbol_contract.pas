program test_cert_utils_generate_selfsigned_rsa_generate_key_ex_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-rsa-generate-key-ex-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenRSAGenerateKeyExIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalRSAGenerateKeyEx: TRSA_generate_key_ex;
begin
  WriteLn;
  WriteLn('=== Certificate utils self-signed RSA_generate_key_ex symbol guard ===');

  if not Assigned(RSA_generate_key_ex) then
  begin
    MarkSkip('certificate utils selfsigned RSA_generate_key_ex contract',
      'required baseline RSA_generate_key_ex helper is unavailable');
    Exit;
  end;
  ...
  LOriginalRSAGenerateKeyEx := RSA_generate_key_ex;
  RSA_generate_key_ex := nil;
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when RSA_generate_key_ex is unavailable',
      LOptions
    );
  finally
    RSA_generate_key_ex := LOriginalRSAGenerateKeyEx;
  end;
end;

begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned RSA_generate_key_ex Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    begin
      ...
      if not LoadOpenSSLRSA() then
        raise Exception.Create('failed to load RSA support');
      if not LoadOpenSSLBN() then
        raise Exception.Create('failed to load BN support');
    end;

    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenRSAGenerateKeyExIsUnavailable;

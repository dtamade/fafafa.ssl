program test_cert_utils_generate_selfsigned_bio_read_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-bio-read-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenBIOReadIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalBIORead: TBIO_read;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSelfSigned BIO_read symbol guard ===');

  if not Assigned(BIO_read) then
  begin
    MarkSkip('certificate utils GenerateSelfSigned BIO_read contract',
  ...
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when BIO_read is unavailable',
      LOptions
    );
  finally
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned BIO_read Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenBIOReadIsUnavailable;

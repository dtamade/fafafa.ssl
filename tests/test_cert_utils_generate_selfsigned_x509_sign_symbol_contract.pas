program test_cert_utils_generate_selfsigned_x509_sign_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-x509-sign-contract.local';
...
procedure TestGenerateSelfSignedShouldFailControlledWhenX509SignIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalX509Sign: TX509_sign;
  LRaised: Boolean;
  LControlled: Boolean;
  LDetail: string;
  LCertPEM: string;
...
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSelfSigned X509_sign symbol guard ===');
  ...
  begin
    MarkSkip('certificate utils selfsigned X509_sign symbol contract',
  ...
  try
    ...
    AssertTrue('GenerateSelfSigned when X509_sign is unavailable should raise',
      LRaised, 'expected GenerateSelfSigned(...) to fail');
    AssertTrue('GenerateSelfSigned when X509_sign is unavailable should raise controlled ESSLCertError',
      LControlled, LDetail);
    ...
    AssertTrue('TryGenerateSelfSigned when X509_sign is unavailable should not raise',
      not LTryRaised, LTryDetail);
    AssertTrue('TryGenerateSelfSigned when X509_sign is unavailable should return False',
      not LTryResult, 'expected TryGenerateSelfSigned to return False');
    AssertTrue('TryGenerateSelfSigned when X509_sign is unavailable should clear cert output',
      LCertPEM = '', 'expected cleared certificate output');
    AssertTrue('TryGenerateSelfSigned when X509_sign is unavailable should clear key output',
    ...
    AssertTrue('TryGenerateSelfSignedSimple when X509_sign is unavailable should not raise',
      not LTrySimpleRaised, LTrySimpleDetail);
    AssertTrue('TryGenerateSelfSignedSimple when X509_sign is unavailable should return False',
      not LTrySimpleResult, 'expected TryGenerateSelfSignedSimple to return False');
    AssertTrue('TryGenerateSelfSignedSimple when X509_sign is unavailable should clear cert output',
      LCertPEM = '', 'expected cleared certificate output');
    AssertTrue('TryGenerateSelfSignedSimple when X509_sign is unavailable should clear key output',
      LKeyPEM = '', 'expected cleared key output');
  finally
    X509_sign := LOriginalX509Sign;
  end;
end;

begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned X509_sign Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailControlledWhenX509SignIsUnavailable;

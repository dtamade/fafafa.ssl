program test_cert_utils_generate_selfsigned_x509_gmtime_adj_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-x509-gmtime-adj-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenX509GmtimeAdjIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalX509GmtimeAdj: TX509_gmtime_adj;
begin
  WriteLn;
  WriteLn('=== Certificate utils self-signed X509_gmtime_adj symbol guard ===');

  if not Assigned(X509_gmtime_adj) then
  begin
    MarkSkip('certificate utils selfsigned X509_gmtime_adj contract',
  ...
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when X509_gmtime_adj is unavailable',
      LOptions
    );
  finally
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned X509_gmtime_adj Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenX509GmtimeAdjIsUnavailable;

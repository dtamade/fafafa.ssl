program test_cert_utils_generate_selfsigned_private_key_bio_read_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-private-key-bio-read-contract.local';
...
function FailOnSecondBIOReadCall(ABIO: Pointer; AData: Pointer; ADLen: Integer): Integer; cdecl;
begin
  Inc(GBIOReadCallCount);
  Result := GOriginalBIORead(ABIO, AData, ADLen);
  if GBIOReadCallCount = 1 then
    BIO_read := nil;
end;

procedure InstallPrivateKeyBIOReadFailureWrapper;
begin
  GBIOReadCallCount := 0;
  BIO_read := @FailOnSecondBIOReadCall;
end;
...
begin
  InstallPrivateKeyBIOReadFailureWrapper;
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenPrivateKeyBIOReadBecomesUnavailable;
var
  LOptions: TCertGenOptions;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSelfSigned private-key BIO_read symbol guard ===');

  if not Assigned(BIO_read) then
  begin
    MarkSkip('certificate utils GenerateSelfSigned private-key BIO_read contract',
      'required baseline BIO_read helper is unavailable');
    Exit;
  end;
  ...
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when private-key BIO_read becomes unavailable',
      LOptions
    );
  finally
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned Private-Key BIO_read Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenPrivateKeyBIOReadBecomesUnavailable;

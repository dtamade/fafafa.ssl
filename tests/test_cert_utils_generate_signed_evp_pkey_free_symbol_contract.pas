program test_cert_utils_generate_signed_evp_pkey_free_symbol_contract;
...
begin
  ...
  Result.CommonName := 'signed-evp-pkey-free-root.local';
...
begin
  Result := TCertificateUtils.DefaultGenOptions;
  Result.CommonName := 'signed-evp-pkey-free-leaf.local';
  Result.Organization := 'fafafa.ssl contract';
  Result.IsCA := False;
  Result.ValidDays := 30;
end;
...
procedure TestGenerateSignedShouldFailGracefullyWhenEVPPKeyFreeIsUnavailable;
var
  LLeafOptions: TCertGenOptions;
  LCACertPEM: string;
  LCAKeyPEM: string;
...
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSigned EVP_PKEY_free symbol guard ===');

  if not Assigned(EVP_PKEY_free) then
  begin
    MarkSkip('certificate utils generate-signed EVP_PKEY_free contract',
      'required baseline EVP_PKEY_free helper is unavailable');
    Exit;
  end;
  ...
  try
    AssertGenerateSignedControlledFailure(
      'GenerateSigned when EVP_PKEY_free is unavailable',
      LLeafOptions,
      LCACertPEM,
      LCAKeyPEM
    );
  finally
    EVP_PKEY_free := LOriginalEVPPKeyFree;
  end;
end;

begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSigned EVP_PKEY_free Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSignedShouldFailGracefullyWhenEVPPKeyFreeIsUnavailable;

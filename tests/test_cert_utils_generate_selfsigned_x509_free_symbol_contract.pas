program test_cert_utils_generate_selfsigned_x509_free_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-x509-free-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenX509FreeIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalX509Free: TX509_free;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSelfSigned X509_free symbol guard ===');

  if not Assigned(X509_free) then
  begin
    MarkSkip('certificate utils GenerateSelfSigned X509_free contract',
  ...
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when X509_free is unavailable',
      LOptions
    );
  finally
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned X509_free Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenX509FreeIsUnavailable;

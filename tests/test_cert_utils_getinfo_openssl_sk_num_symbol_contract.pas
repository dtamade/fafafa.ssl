program test_cert_utils_getinfo_openssl_sk_num_symbol_contract;
...
procedure AssertOpenSSLStackCountGuardInfo(
  const AName: string;
...
begin
  ...
  begin
    try
      AssertOpenSSLStackCountGuardInfo(
        AName,
        LInfo,
        AExpectedSubject,
        AExpectedIssuer,
...
procedure TestGetInfoShouldFailGracefullyWhenOpenSSLStackCountIsUnavailable;
var
  LFixturePEM: string;
  ...
  LOriginalOpenSSLStackNum: TOPENSSL_sk_num;
begin
  WriteLn;
  WriteLn('=== Certificate utils GetInfo OPENSSL_sk_num symbol guard ===');
  ...
  if (not Assigned(BIO_new_mem_buf)) or
     ...
     (not Assigned(OPENSSL_sk_num)) then
  begin
    MarkSkip('certificate utils getinfo OPENSSL_sk_num symbol contract',
  ...
  LOriginalOpenSSLStackNum := OPENSSL_sk_num;
  OPENSSL_sk_num := nil;
  try
    AssertGetInfoSafeDegrade(
      'GetInfo when OPENSSL_sk_num is unavailable',
      LFixturePEM,
      LExpectedSubject,
      LExpectedIssuer,
      LExpectedVersion,
    ...
    OPENSSL_sk_num := LOriginalOpenSSLStackNum;
  end;
end;

begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GetInfo OPENSSL_sk_num Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGetInfoShouldFailGracefullyWhenOpenSSLStackCountIsUnavailable;

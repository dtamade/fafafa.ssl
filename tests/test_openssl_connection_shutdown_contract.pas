program test_openssl_connection_shutdown_contract;
...
begin
  WriteLn;
  WriteLn('=== OpenSSL connection shutdown guard ===');

  if (not Assigned(SSL_shutdown)) or
     (not Assigned(SSL_new)) or
     (not Assigned(SSL_set_bio)) or
     (not Assigned(BIO_new)) or
     (not Assigned(BIO_s_mem)) then
  begin
    MarkSkip('openssl connection shutdown contract',
  ...
  try
    ...
    try
      ...
      SSL_shutdown := LOriginalSSLShutdown;
    end;

    AssertTrue('Shutdown when SSL_shutdown is unavailable should not raise',
      not LRaised, LDetail);
    AssertTrue('Shutdown when SSL_shutdown is unavailable should return True',
      LResult, 'expected Shutdown to preserve its True return contract');
  finally
    if Assigned(LConn) then
      LConn.Free;
    LStream.Free;
...
begin
  WriteLn('========================================');
  WriteLn('OpenSSL Connection Shutdown Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestShutdownShouldRemainSafeWhenSSLShutdownHelperIsUnavailable;

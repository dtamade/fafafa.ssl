program test_cert_utils_generate_selfsigned_addextension_x509v3_set_ctx_symbol_contract;
...
type
  TX509V3SetCtxProc = procedure(ctx: PX509V3_CTX; issuer: Pointer; subject: Pointer;
    req: Pointer; crl: Pointer; flags: Integer); cdecl;
...
begin
  ...
  Result.CommonName := 'selfsigned-x509v3-set-ctx-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenX509V3SetCtxIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalX509V3SetCtx: TX509V3SetCtxProc;
begin
  WriteLn;
  WriteLn('=== Certificate utils self-signed AddExtension X509V3_set_ctx symbol guard ===');

  if not Assigned(X509V3_set_ctx) then
  begin
    MarkSkip('certificate utils selfsigned AddExtension X509V3_set_ctx contract',
      'required baseline X509V3_set_ctx helper is unavailable');
    Exit;
  end;
  ...
  LOriginalX509V3SetCtx := X509V3_set_ctx;
  X509V3_set_ctx := nil;
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when X509V3_set_ctx is unavailable',
      LOptions
    );
  finally
    X509V3_set_ctx := LOriginalX509V3SetCtx;
  end;
end;

begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned AddExtension X509V3_set_ctx Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenX509V3SetCtxIsUnavailable;

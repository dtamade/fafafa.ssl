program test_cert_utils_generate_signed_x509_new_symbol_contract;
...
begin
  ...
  Result.CommonName := 'signed-x509-new-root.local';
...
begin
  Result := TCertificateUtils.DefaultGenOptions;
  Result.CommonName := 'signed-x509-new-leaf.local';
  Result.Organization := 'fafafa.ssl contract';
  Result.IsCA := False;
  Result.ValidDays := 30;
end;
...
procedure TestGenerateSignedShouldFailGracefullyWhenX509NewIsUnavailable;
var
  LLeafOptions: TCertGenOptions;
  LCACertPEM: string;
  LCAKeyPEM: string;
  LOriginalX509New: TX509_new;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSigned X509_new symbol guard ===');

  if not Assigned(X509_new) then
  begin
    MarkSkip('certificate utils generate-signed X509_new contract',
      'required baseline X509_new helper is unavailable');
    Exit;
  end;
  ...
  LOriginalX509New := X509_new;
  X509_new := nil;
  try
    AssertGenerateSignedControlledFailure(
      'GenerateSigned when X509_new is unavailable',
      LLeafOptions,
      LCACertPEM,
      LCAKeyPEM
    );
  finally
    X509_new := LOriginalX509New;
  end;
end;

begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSigned X509_new Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSignedShouldFailGracefullyWhenX509NewIsUnavailable;

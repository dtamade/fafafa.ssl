program test_tls12_recordcrypto_aes_gcm;
...
procedure TestAES128GCMRoundTrip;
var
  LKey: TBytes;
  LWriteIV: TBytes;
  LPlaintext: TBytes;
  ...
  LError: string;
begin
  LKey := HexToBytes('11111111111111111111111111111111');
  ...
  AssertTrue(
    ...
    'TLS1.2 AES128-GCM encrypt should succeed: ' + LError
  );
  AssertTrue(Length(LEncrypted) = 8 + Length(LPlaintext) + 16,
    'TLS1.2 AES128-GCM encrypted payload length mismatch');
  ...
  AssertTrue(
    ...
    'TLS1.2 AES128-GCM decrypt should succeed: ' + LError
  );
  AssertBytesEqual(LPlaintext, LDecrypted, 'TLS1.2 AES128-GCM roundtrip plaintext mismatch');
end;

begin
  WriteLn('Testing TLS 1.2 AES128-GCM record crypto helpers...');
  TestAES128GCMRoundTrip;
  WriteLn('✅ TLS 1.2 AES128-GCM record crypto checks passed');
end.

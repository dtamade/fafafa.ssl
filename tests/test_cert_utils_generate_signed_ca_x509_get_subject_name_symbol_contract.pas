program test_cert_utils_generate_signed_ca_x509_get_subject_name_symbol_contract;
...
var
  ...
  GX509GetSubjectNameCallCount: Integer = 0;
...
begin
  ...
  Result.CommonName := 'signed-ca-x509-get-subject-name-root.local';
...
begin
  Result := TCertificateUtils.DefaultGenOptions;
  Result.CommonName := 'signed-ca-x509-get-subject-name-leaf.local';
  Result.Organization := 'fafafa.ssl contract';
  Result.IsCA := False;
  Result.ValidDays := 30;
end;
...
function FailOnSecondX509GetSubjectNameCall(const ACert: Pointer): Pointer; cdecl;
begin
  Inc(GX509GetSubjectNameCallCount);
  Result := GOriginalX509GetSubjectName(ACert);
  if GX509GetSubjectNameCallCount = 1 then
    X509_get_subject_name := nil;
end;

procedure InstallCAGetSubjectNameFailureWrapper;
begin
  GX509GetSubjectNameCallCount := 0;
  X509_get_subject_name := @FailOnSecondX509GetSubjectNameCall;
end;
...
var
  ...
  LTryResult: Boolean;
begin
  InstallCAGetSubjectNameFailureWrapper;
...
procedure TestGenerateSignedShouldFailGracefullyWhenCAGetSubjectNameBecomesUnavailable;
var
  LLeafOptions: TCertGenOptions;
  LCACertPEM: string;
  LCAKeyPEM: string;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSigned CA X509_get_subject_name symbol guard ===');

  if not Assigned(X509_get_subject_name) then
  begin
    MarkSkip('certificate utils generate-signed CA X509_get_subject_name contract',
      'required baseline X509_get_subject_name helper is unavailable');
    Exit;
  end;
  ...
  try
    AssertGenerateSignedControlledFailure(
      'GenerateSigned when CA X509_get_subject_name becomes unavailable',
      LLeafOptions,
      LCACertPEM,
      LCAKeyPEM
    );
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSigned CA X509_get_subject_name Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSignedShouldFailGracefullyWhenCAGetSubjectNameBecomesUnavailable;

program test_cert_utils_generate_selfsigned_bn_set_word_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-bn-set-word-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenBNSetWordIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalBNSetWord: TBN_set_word;
begin
  WriteLn;
  WriteLn('=== Certificate utils self-signed BN_set_word symbol guard ===');

  if not Assigned(BN_set_word) then
  begin
    MarkSkip('certificate utils selfsigned BN_set_word contract',
      'required baseline BN_set_word helper is unavailable');
    Exit;
  end;
  ...
  LOriginalBNSetWord := BN_set_word;
  BN_set_word := nil;
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when BN_set_word is unavailable',
      LOptions
    );
  finally
    BN_set_word := LOriginalBNSetWord;
  end;
end;

begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned BN_set_word Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenBNSetWordIsUnavailable;

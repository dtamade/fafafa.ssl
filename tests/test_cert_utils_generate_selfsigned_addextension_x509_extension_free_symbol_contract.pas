program test_cert_utils_generate_selfsigned_addextension_x509_extension_free_symbol_contract;
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenX509ExtensionFreeIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalX509ExtensionFree: TX509_EXTENSION_free;
begin
  WriteLn;
  WriteLn('=== Certificate utils self-signed AddExtension X509_EXTENSION_free symbol guard ===');

  if not Assigned(X509_EXTENSION_free) then
  begin
    MarkSkip('certificate utils selfsigned AddExtension X509_EXTENSION_free contract',
      'required baseline X509_EXTENSION_free helper is unavailable');
    Exit;
  end;
  ...
  LOriginalX509ExtensionFree := X509_EXTENSION_free;
  X509_EXTENSION_free := nil;
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when X509_EXTENSION_free is unavailable',
      LOptions
    );
  finally
    X509_EXTENSION_free := LOriginalX509ExtensionFree;
  end;
end;

begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned AddExtension X509_EXTENSION_free Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenX509ExtensionFreeIsUnavailable;

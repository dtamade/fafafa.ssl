...
uses
  SysUtils, fafafa.ssl.tls12.wire, fafafa.ssl.tls12.clienthello;
...
begin
  ...
  Check(LCipherLen = 8, 'Cipher suite list should be 8 bytes (4 suites)');

  LFirstSuite := (Word(LHello[LOffset + 2]) shl 8) or Word(LHello[LOffset + 3]);
  Check(LFirstSuite = TLS12_CIPHER_ECDHE_RSA_WITH_AES_128_GCM_SHA256,
    'First cipher suite should be ECDHE_RSA_AES_128_GCM_SHA256');
end;

program test_cert_utils_generate_selfsigned_evp_pkey_assign_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-evp-pkey-assign-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenEVPPKeyAssignIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalEVPPKeyAssign: TEVP_PKEY_assign;
begin
  WriteLn;
  WriteLn('=== Certificate utils self-signed EVP_PKEY_assign symbol guard ===');

  if not Assigned(EVP_PKEY_assign) then
  begin
    MarkSkip('certificate utils selfsigned EVP_PKEY_assign contract',
      'required baseline EVP_PKEY_assign helper is unavailable');
    Exit;
  end;
  ...
  LOriginalEVPPKeyAssign := EVP_PKEY_assign;
  EVP_PKEY_assign := nil;
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when EVP_PKEY_assign is unavailable',
      LOptions
    );
  finally
    EVP_PKEY_assign := LOriginalEVPPKeyAssign;
  end;
end;

begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned EVP_PKEY_assign Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenEVPPKeyAssignIsUnavailable;

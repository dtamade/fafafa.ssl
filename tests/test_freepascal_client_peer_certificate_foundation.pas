program test_freepascal_client_peer_certificate_foundation;
...
procedure TestClientCapturesPeerCertificate;
var
...
begin
  ...
  try
    ...
    AssertTrue(LClientConn.Connect,
      'Client connect should complete using scripted server flight: ' +
      LClientConn.GetVerifyResultString + ' err=' + IntToStr(Ord(LClientConn.GetError(-1))) +
      ' state=' + LClientConn.GetStateString);
    AssertTrue(LClientConn.GetSelectedALPNProtocol = 'h2',
      'Client should expose negotiated ALPN from EncryptedExtensions');
    LInfo := LClientConn.GetConnectionInfo;
    AssertTrue(LInfo.ALPNProtocol = 'h2',
      'Connection info should expose negotiated ALPN');
    AssertTrue(LInfo.PeerCertificate.Subject <> '',
      'Connection info should expose peer certificate subject');
    AssertTrue(LInfo.PeerCertificate.FingerprintSHA256 <> '',
      'Connection info should expose peer certificate fingerprint');
    AssertTrue(LInfo.PeerCertificate.PublicKeyAlgorithm <> '',
      'Connection info should expose peer certificate public key algorithm');

    LPeerCert := LClientConn.GetPeerCertificate;
    AssertTrue(LPeerCert <> nil, 'Client should expose peer certificate after handshake');
    AssertTrue(LPeerCert.GetFingerprintSHA256 <> '',
      'Peer certificate should expose non-empty fingerprint');

    LPeerChain := LClientConn.GetPeerCertificateChain;
    AssertTrue(Length(LPeerChain) > 0, 'Client should expose peer certificate chain after handshake');
    AssertTrue(LPeerChain[0] <> nil, 'Peer certificate chain should contain non-nil leaf certificate');
  finally
...
begin
  WriteLn('Testing FreePascal client peer certificate foundation...');
  TestClientCapturesPeerCertificate;
  WriteLn('✅ FreePascal client peer certificate foundation checks passed');
end.

program test_cert_utils_generate_signed_x509_set_pubkey_symbol_contract;
...
begin
  ...
  Result.CommonName := 'signed-x509-set-pubkey-root.local';
...
begin
  Result := TCertificateUtils.DefaultGenOptions;
  Result.CommonName := 'signed-x509-set-pubkey-leaf.local';
  Result.Organization := 'fafafa.ssl contract';
  Result.IsCA := False;
  Result.ValidDays := 30;
end;
...
procedure TestGenerateSignedShouldFailGracefullyWhenX509SetPubkeyIsUnavailable;
var
  LLeafOptions: TCertGenOptions;
  LCACertPEM: string;
  LCAKeyPEM: string;
  LOriginalX509SetPubkey: TX509_set_pubkey;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSigned X509_set_pubkey symbol guard ===');

  if not Assigned(X509_set_pubkey) then
  begin
    MarkSkip('certificate utils generate-signed X509_set_pubkey contract',
      'required baseline X509_set_pubkey helper is unavailable');
    Exit;
  end;
  ...
  LOriginalX509SetPubkey := X509_set_pubkey;
  X509_set_pubkey := nil;
  try
    AssertGenerateSignedControlledFailure(
      'GenerateSigned when X509_set_pubkey is unavailable',
      LLeafOptions,
      LCACertPEM,
      LCAKeyPEM
    );
  finally
    X509_set_pubkey := LOriginalX509SetPubkey;
  end;
end;

begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSigned X509_set_pubkey Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSignedShouldFailGracefullyWhenX509SetPubkeyIsUnavailable;

...
WORK_DIR="$(mktemp -d -t fafafa_fp_tls12_alpn_XXXXXX)"
PORT=18485
...
echo "[TEST] pure Pascal TLS1.2 local OpenSSL ALPN contract"

require_cmd openssl
require_cmd fpc

cat > "$WORK_DIR/fp_tls12_alpn_probe.pas" <<'PAS'
program fp_tls12_alpn_probe;
...
begin
  Sock := ConnectTCP('127.0.0.1', 18485);
...
(
  ...
  fpc -B -Mobjfpc -Sh \
    ...
    "$WORK_DIR/fp_tls12_alpn_probe.pas" \
    -o"$WORK_DIR/fp_tls12_alpn_probe" >/dev/null
)
...
(
  cd "$ROOT_DIR"
  "$WORK_DIR/fp_tls12_alpn_probe" >"$CLIENT_LOG" 2>&1
) || {
  echo "--- client log ---"
  cat "$CLIENT_LOG" || true
  echo "--- server log (tail) ---"
  tail -n 120 "$SERVER_LOG" || true
  fail "pure Pascal TLS1.2 ALPN probe should succeed"
}

rg -F --quiet -- "ALPN_OK" "$CLIENT_LOG" || {
  cat "$CLIENT_LOG"
  fail "TLS1.2 ALPN probe should report ALPN_OK"
}

rg -F --quiet -- "ALPN=h2 INFO_ALPN=h2" "$CLIENT_LOG" || {
  cat "$CLIENT_LOG"
  fail "TLS1.2 ALPN probe should expose negotiated h2 on both APIs"
}

echo "[PASS] pure Pascal TLS1.2 local OpenSSL ALPN contract passed"

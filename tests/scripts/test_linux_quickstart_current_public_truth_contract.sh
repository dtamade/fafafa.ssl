...
require_fixed "$guide" "fafafa.ssl.pas                # 主门面 / 当前普通入口" \
  "LINUX_QUICKSTART project structure must show the current public facade"
require_fixed "$guide" "fafafa.ssl.context.builder.pas # 推荐 context builder 入口" \
  "LINUX_QUICKSTART project structure must show the current builder entrypoint"
require_fixed "$guide" "fafafa.ssl.openssl.backed.pas # OpenSSL ISSLLibrary 实现" \
...
require_absent "$guide" "  fafafa.ssl.factory;" \
...
require_absent "$guide" "fafafa.ssl.openssl.pas" \
  "LINUX_QUICKSTART must stop naming the nonexistent fafafa.ssl.openssl.pas unit"

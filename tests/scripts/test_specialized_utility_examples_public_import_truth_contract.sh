...
require_fixed "$example_cert_pinning" "  fafafa.ssl.cert.pinning;" \
...
require_fixed "$example_error_handling" "  fafafa.ssl.exceptions;" \
...
require_fixed "$example_result_type" "  fafafa.ssl.crypto.utils," \
...
require_fixed "$example_streaming_operations" "  fafafa.ssl.crypto.utils," \
  "example_streaming_operations must keep crypto owner unit"
require_fixed "$example_streaming_operations" "  fafafa.ssl.encoding;" \
...
require_fixed "$test_ssl_context" "  fafafa.ssl.openssl.backed," \
...
require_fixed "$generate_certificate" "  fafafa.ssl.openssl.backed," \
...
require_absent "$winssl_fips" "fafafa.ssl.factory," \
  "09_winssl_fips must stop printing factory split-import guidance"
require_absent "$winssl_fips" "fafafa.ssl.base;" \

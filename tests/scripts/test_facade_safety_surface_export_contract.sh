...
require_fixed "$facade" "TSSLVersion = fafafa.ssl.safety.TSSLVersion;" \
  "main facade must re-export TSSLVersion"
require_fixed "$facade" "TKeySize = fafafa.ssl.safety.TKeySize;" \
...
require_fixed "$api_ref" '`fafafa.ssl` 主门面当前也 re-export 这组 non-generic type-safety public surface（如 `TSSLVersion` / `TKeySize` / `TTimeoutDuration` / `TBufferSize`）；`TSecureData<T>` / `TResult<T, E>` 继续保留在 `fafafa.ssl.safety`。' \
  "API reference must record the main facade type-safety surface coverage"
require_fixed "$readme" '`fafafa.ssl` 主门面当前也 re-export `TSSLVersion` / `TKeySize` / `TTimeoutDuration` / `TBufferSize` 这组 non-generic type-safety surface；`TSecureData<T>` / `TResult<T, E>` 继续保留在 `fafafa.ssl.safety`。' \
  "README must mention the current facade type-safety surface"
require_fixed "$migration" '如果你在新代码里要直接使用 `TSSLVersion` / `TKeySize` / `TTimeoutDuration` / `TBufferSize`，当前可以直接从 `fafafa.ssl` 取得；若你要使用 `TSecureData<T>` / `TResult<T, E>` 这组 generic pattern，则继续单独 `uses fafafa.ssl.safety`。' \

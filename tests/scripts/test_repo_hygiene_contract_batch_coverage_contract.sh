...
BATCH_SCRIPT="$ROOT_DIR/tests/scripts/test_repo_hygiene_contract_batch.sh"
...
echo "[TEST] repo hygiene contract batch coverage contract"

[[ -f "$BATCH_SCRIPT" ]] || fail "missing batch script: tests/scripts/test_repo_hygiene_contract_batch.sh"
...
for rel in "${expected[@]}"; do
  grep -Fq "$rel" "$BATCH_SCRIPT" || fail "batch script should include $rel"
done

echo "[PASS] repo hygiene contract batch coverage contract passed"

...
WORK_REL="tmp/test_prepare_wave_b_b2_handoff_bundle_linux_next_actions_$(date +%s)"
WORK_DIR="$ROOT_DIR/$WORK_REL"
RUN_ID="handoff_linux_next_actions_truth"
...
if ! rg -n "Linux.*FAIL" "$REPORT_ABS" >/dev/null; then
...
if ! rg -n "prepare_wave_b_b2_handoff_bundle\\.sh .* --strict" "$REPORT_ABS" >/dev/null; then
  fail "handoff bundle should still keep the replay prepare command"
fi

echo "[PASS] prepare_wave_b_b2 handoff bundle linux next actions contract passed"

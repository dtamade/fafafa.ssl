...
if ! rg -F --quiet -- 'fafafa.ssl.winssl.lib' "$FILE"; then
  fail "test_backend_comparison.pas should import fafafa.ssl.winssl.lib so Windows factory tests can register WinSSL"
fi

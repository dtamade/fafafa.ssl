...
if ! rg -n --quiet 'fafafa\.ssl\.pas\s+# 主门面 / 当前普通入口' README.md; then
  fail "README architecture entry must publish fafafa.ssl.pas as the ordinary public entry"
fi

if ! rg -n --quiet 'fafafa\.ssl\.context\.builder\.pas\s+# 推荐 context builder 入口' README.md; then
  fail "README architecture entry must publish fafafa.ssl.context.builder.pas as the recommended builder entry"
fi

if rg -n --quiet 'fafafa\.ssl\.factory\.pas\s+# 工厂模式入口' README.md; then
  fail "README must not keep publishing fafafa.ssl.factory.pas as the primary entry"
fi
pass "README root entry guidance aligned"

if rg -n --quiet 'fafafa\.ssl,\s*fafafa\.ssl\.base' docs/zh/安装配置.md; then
  fail "zh install guide must not require split import fafafa.ssl + fafafa.ssl.base for ordinary setup"
fi
...
if rg -n --quiet 'fafafa\.ssl\.base;\s*// 或直接使用 base 单元' docs/zh/编译指南.md; then
  fail "zh compile guide must not recommend fafafa.ssl.base as the default companion import"
fi

...
WORK_DIR="$(mktemp -d -t fafafa_fp_reneg_retry_XXXXXX)"
BACKEND_PORT=18453
PROXY_PORT=18452
...
echo "[TEST] pure Pascal renegotiate nonblocking retry contract"
...
cat > "$WORK_DIR/fp_reneg_retry_proxy.py" <<'PY'
import socket
import sys
import threading
import time
...
cat > "$WORK_DIR/fp_reneg_retry_probe.pas" <<'PAS'
program fp_reneg_retry_probe;
{$mode ObjFPC}{$H+}
...
begin
  Sock := ConnectTCP('127.0.0.1', 18452);
  ...
  if Conn.Renegotiate then
  begin
    WriteLn('RENEGOTIATE_UNEXPECTED_IMMEDIATE');
    Halt(2);
  end;

  WriteLn(
    'RENEGOTIATE_BLOCKED err=', Ord(Conn.GetError(-1)),
    ' wantwrite=', BoolToStr(Conn.WantWrite, True),
    ' detail=', Conn.GetVerifyResultString
  );
  ...
  begin
    if Conn.Renegotiate then
    begin
      WriteLn('RENEGOTIATE_RESUME_OK');
      Halt(0);
    end;
    WriteLn(
      'RETRY_RENEGOTIATE', RetryAttempt,
  ...
  WriteLn('RENEGOTIATE_RESUME_MISSING');
  Halt(3);
end.
PAS
...
(
  ...
  fpc -B -Mobjfpc -Sh -Fu./src -Fi./src -Fu./examples -FU./lib \
    "$WORK_DIR/fp_reneg_retry_probe.pas" \
    -o"$WORK_DIR/fp_reneg_retry_probe" >/dev/null
)
...
python3 "$WORK_DIR/fp_reneg_retry_proxy.py" "$PROXY_PORT" "$BACKEND_PORT" \
  >"$PROXY_LOG" 2>&1 &
PROXY_PID=$!
sleep 0.5
...
(
  cd "$ROOT_DIR"
  timeout 12 "$WORK_DIR/fp_reneg_retry_probe" >"$PROBE_LOG" 2>&1
) || true

if ! rg -F --quiet -- "RENEGOTIATE_BLOCKED err=18 wantwrite=True detail=Previous TLS write is still pending; retry the same operation" "$PROBE_LOG"; then
  echo "--- probe log ---"
  cat "$PROBE_LOG" || true
  fail "renegotiate should surface WantWrite while a previous TLS write is still pending"
fi

if ! rg -F --quiet -- "RENEGOTIATE_RESUME_OK" "$PROBE_LOG"; then
  echo "--- probe log ---"
  cat "$PROBE_LOG" || true
  fail "renegotiate should succeed after the pending write is resumed"
fi

echo "[PASS] pure Pascal renegotiate nonblocking retry contract passed"

...
require_fixed "$architecture" '`fafafa.ssl.openssl.*`    | OpenSSL 实现（Linux/macOS 默认）' \
...
require_fixed "$architecture" '`fafafa.ssl.winssl.*`     | Windows Schannel 实现（Windows 默认）' \
...
require_absent "$architecture" '| `fafafa.ssl.openssl.*` | OpenSSL 实现（Linux/macOS 默认） | 默认启用 | ✅ 生产就绪 |' \

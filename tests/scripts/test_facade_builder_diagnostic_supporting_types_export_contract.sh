...
require_fixed "$facade" "TBuildValidationResult = fafafa.ssl.base.TBuildValidationResult;" \
  "main facade must re-export TBuildValidationResult"
require_fixed "$facade" "TSSLErrorRecord = fafafa.ssl.base.TSSLErrorRecord;" \
  "main facade must re-export TSSLErrorRecord"
require_fixed "$api_ref" '主门面 `fafafa.ssl` 当前也 re-export `TBuildValidationResult`；使用 `fafafa.ssl.context.builder` 时不需要回退 `fafafa.ssl.base` 才能接住 `Validate*` / `Build*WithValidation(...)` 的结果。' \
  "API reference must record the main-facade builder-validation supporting type"
require_fixed "$api_ref" '主门面 `fafafa.ssl` 当前也 re-export `TSSLErrorRecord`；与 `TSSLDiagnosticInfo` 一起使用时不需要回退 `fafafa.ssl.base`。' \

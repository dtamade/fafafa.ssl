...
WORK_DIR="$(mktemp -d -t fafafa_fp_tls12_client_no_alpn_XXXXXX)"
PORT=18491
...
echo "[TEST] pure Pascal TLS1.2 client-no-offer ALPN contract"

require_cmd openssl
require_cmd fpc

cat > "$WORK_DIR/fp_tls12_client_no_alpn_offer_probe.pas" <<'PAS'
program fp_tls12_client_no_alpn_offer_probe;
...
begin
  Sock := ConnectTCP('127.0.0.1', 18491);
  ...
  WriteLn(
    'ALPN_OK PROTO=', Ord(Conn.GetProtocolVersion),
    ' CIPHER=', Conn.GetCipherName,
    ' ALPN=', ShowALPN(Conn.GetSelectedALPNProtocol),
...
(
  ...
  fpc -B -Mobjfpc -Sh \
    ...
    "$WORK_DIR/fp_tls12_client_no_alpn_offer_probe.pas" \
    -o"$WORK_DIR/fp_tls12_client_no_alpn_offer_probe" >/dev/null
)
...
(
  cd "$ROOT_DIR"
  "$WORK_DIR/fp_tls12_client_no_alpn_offer_probe" >"$CLIENT_LOG" 2>&1
) || {
  echo "--- client log ---"
  cat "$CLIENT_LOG" || true
  echo "--- server log (tail) ---"
  tail -n 120 "$SERVER_LOG" || true
  fail "pure Pascal TLS1.2 client-no-offer ALPN probe should succeed"
}

rg -F --quiet -- "ALPN_OK" "$CLIENT_LOG" || {
  cat "$CLIENT_LOG"
  fail "TLS1.2 client-no-offer probe should report ALPN_OK"
}

rg -F --quiet -- "ALPN=<empty> INFO_ALPN=<empty>" "$CLIENT_LOG" || {
  cat "$CLIENT_LOG"
  fail "TLS1.2 client-no-offer probe should expose empty negotiated ALPN on both APIs"
}

echo "[PASS] pure Pascal TLS1.2 client-no-offer ALPN contract passed"

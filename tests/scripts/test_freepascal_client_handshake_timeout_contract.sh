...
WORK_DIR="$(mktemp -d -t fafafa_fp_hs_timeout_XXXXXX)"
PORT=18459
...
echo "[TEST] pure Pascal client handshake timeout contract"
...
cat > "$WORK_DIR/fp_client_handshake_timeout_probe.pas" <<'PAS'
program fp_client_handshake_timeout_probe;
{$mode ObjFPC}{$H+}
...
var
  ...
  Sock: TSocketHandle;
begin
  Sock := ConnectTCP('127.0.0.1', 18459);
...
(
  ...
  fpc -B -Mobjfpc -Sh -Fu./src -Fi./src -Fu./examples -FU./lib \
    "$WORK_DIR/fp_client_handshake_timeout_probe.pas" -o"$WORK_DIR/fp_client_handshake_timeout_probe" >/dev/null
)
...
(
  cd "$ROOT_DIR"
  timeout 5 "$WORK_DIR/fp_client_handshake_timeout_probe" >"$PROBE_LOG" 2>&1
) || true

if ! rg -F --quiet -- "CONNECT_FAIL" "$PROBE_LOG" || \
   ! rg -F --quiet -- "IS_TIMEOUT=True" "$PROBE_LOG" || \
   ! rg -F --quiet -- "WANTREAD=False" "$PROBE_LOG"; then
  echo "--- probe log ---"
  cat "$PROBE_LOG" || true
  fail "client handshake timeout should surface sslErrTimeout"
fi

echo "[PASS] pure Pascal client handshake timeout contract passed"

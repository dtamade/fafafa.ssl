...
require_fixed "$facade" "TBytesView = fafafa.ssl.base.TBytesView;" \
  "main facade must re-export TBytesView"
require_fixed "$api_ref" '主门面 `fafafa.ssl` 当前也 re-export `TBytesView`；使用 `fafafa.ssl.encoding` / `fafafa.ssl.crypto.utils` 的 zero-copy 入口时不需要回退 `fafafa.ssl.base`。' \

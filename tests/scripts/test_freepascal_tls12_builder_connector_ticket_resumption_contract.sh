...
WORK_DIR="$(mktemp -d -t fafafa_fp_tls12_builder_ticket_XXXXXX)"
PORT=18495
...
echo "[TEST] pure Pascal TLS1.2 builder/connector ticket-resumption contract"

require_cmd openssl
require_cmd fpc

cat > "$WORK_DIR/fp_tls12_builder_ticket_probe.pas" <<'PAS'
program fp_tls12_builder_ticket_probe;
...
begin
  ...
  Sock := ConnectTCP('127.0.0.1', 18495);
  ...
  try
    ...
    begin
      ReadN := Stream.Read(Buf[0], SizeOf(Buf));
      if ReadN <= 0 then
        Halt(11);
...
(
  ...
  fpc -B -Mobjfpc -Sh \
    ...
    "$WORK_DIR/fp_tls12_builder_ticket_probe.pas" \
    -o"$WORK_DIR/fp_tls12_builder_ticket_probe" >/dev/null
)
...
(
  cd "$ROOT_DIR"
  "$WORK_DIR/fp_tls12_builder_ticket_probe" >"$CLIENT_LOG" 2>&1
) || {
  echo "--- client log ---"
  cat "$CLIENT_LOG" || true
  echo "--- server log (tail) ---"
  tail -n 120 "$SERVER_LOG" || true
  fail "pure Pascal TLS1.2 builder/connector ticket-resumption probe should succeed"
}

rg -F --quiet -- "FIRST_OK REUSED=False RESUMABLE=True" "$CLIENT_LOG" || {
  cat "$CLIENT_LOG"
  fail "first builder/connector TLS1.2 ticket handshake should produce resumable session"
}

rg -F --quiet -- "SECOND_OK REUSED=True RESUMABLE=True" "$CLIENT_LOG" || {
  cat "$CLIENT_LOG"
  fail "second builder/connector TLS1.2 ticket handshake should reuse session"
}

echo "[PASS] pure Pascal TLS1.2 builder/connector ticket-resumption contract passed"

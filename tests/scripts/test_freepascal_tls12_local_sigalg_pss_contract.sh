...
WORK_DIR="$(mktemp -d -t fafafa_fp_tls12_sigpss_XXXXXX)"
PORT=18476
...
echo "[TEST] pure Pascal TLS1.2 local rsa_pss_rsae_sha256 contract"

require_cmd openssl
require_cmd fpc

cat > "$WORK_DIR/fp_tls12_sigpss_probe.pas" <<'PAS'
program fp_tls12_sigpss_probe;
...
var
  ...
  Sock: TSocketHandle;
begin
  Sock := ConnectTCP('127.0.0.1', 18476);
  ...
  WriteLn('SIGPSS_OK CIPHER=', Conn.GetCipherName);
end.
PAS
...
(
  ...
  fpc -B -Mobjfpc -Sh \
    ...
    "$WORK_DIR/fp_tls12_sigpss_probe.pas" \
    -o"$WORK_DIR/fp_tls12_sigpss_probe" >/dev/null
)
...
(
  cd "$ROOT_DIR"
  "$WORK_DIR/fp_tls12_sigpss_probe" >"$CLIENT_LOG" 2>&1
)
STATUS=$?
set -e
...
if [[ "$STATUS" -ne 0 ]]; then
  ...
  fail "pure Pascal TLS1.2 rsa_pss_rsae_sha256 probe should succeed"
fi

rg -F --quiet -- "SIGPSS_OK" "$CLIENT_LOG" || {
  cat "$CLIENT_LOG"
  fail "rsa_pss_rsae_sha256 probe should report SIGPSS_OK"
}

echo "[PASS] pure Pascal TLS1.2 local rsa_pss_rsae_sha256 contract passed"

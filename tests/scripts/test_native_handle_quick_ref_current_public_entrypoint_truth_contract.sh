...
require_contains '普通 capability / native-handle 查询不必再拆分回 `uses fafafa.ssl.base` / `fafafa.ssl.factory`；`fafafa.ssl` 已 re-export `ISSLContext` / `ISSLNativeHandleAccess` / `TSSLFactory`。'

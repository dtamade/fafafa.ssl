...
WORK_REL="tmp/test_wave_c_b138_only_platform_passthrough"
...
echo "[TEST] wave c b138 oncall only-platform passthrough contract"

rm -rf "$WORK_DIR"
mkdir -p "$WORK_DIR/reports"

RUN_ONLY="b138_only_platform_$$"
OUT_ONLY_REL="$WORK_REL/reports/full_gate_only.md"
...
[[ -f "$OUT_ONLY" ]] || fail "only mode should generate b138 report"
...
echo "[PASS] wave c b138 oncall only-platform passthrough contract passed"

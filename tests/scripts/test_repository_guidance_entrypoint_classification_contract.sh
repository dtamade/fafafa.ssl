...
require_fixed "$WINSSL_QUICKSTART" "│       │   ├── fafafa.ssl.pas                # 主门面 / 当前普通入口" \
  "WINSSL_QUICKSTART tree must show the current public facade entry"
require_fixed "$WINSSL_QUICKSTART" "│       │   ├── fafafa.ssl.context.builder.pas # 推荐 context builder 入口" \
  "WINSSL_QUICKSTART tree must show the current builder entry"
require_fixed "$WINSSL_QUICKSTART" "│       │   ├── fafafa.ssl.factory.pas        # core factory surface / direct-library helper" \
  "WINSSL_QUICKSTART tree must classify fafafa.ssl.factory.pas correctly"
require_fixed "$WINSSL_QUICKSTART" "│       │   ├── fafafa.ssl.base.pas           # 底层 source truth / supporting types" \
  "WINSSL_QUICKSTART tree must classify fafafa.ssl.base.pas as source-truth owner"
require_absent_regex "$WINSSL_QUICKSTART" '^│       │   ├── fafafa\.ssl\.base\.pas$' \
  "WINSSL_QUICKSTART must stop leaving fafafa.ssl.base.pas unlabeled in the structure snippet"

require_fixed "$AGENTS_DOC" '主门面入口是 `src/fafafa.ssl.pas`，推荐 builder 入口是 `src/fafafa.ssl.context.builder.pas`；`src/fafafa.ssl.base.pas` 主要承载底层 source truth / supporting types。' \
  "docs/AGENTS must state the current facade / builder / base role split"
require_absent "$AGENTS_DOC" '公共抽象在 `fafafa.ssl.base`' \

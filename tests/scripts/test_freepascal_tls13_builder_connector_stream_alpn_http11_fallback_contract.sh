...
WORK_DIR="$(mktemp -d -t fafafa_fp_tls13_alpn_http11_XXXXXX)"
PORT=18486
...
echo "[TEST] pure Pascal TLS1.3 ALPN http/1.1 fallback contract"
...
cat > "$WORK_DIR/fp_tls13_alpn_http11_probe.pas" <<'PAS'
program fp_tls13_alpn_http11_probe;
...
begin
  Sock := ConnectTCP('127.0.0.1', 18486);
...
(
  ...
  fpc -B -Mobjfpc -Sh \
    ...
    "$WORK_DIR/fp_tls13_alpn_http11_probe.pas" \
    -o"$WORK_DIR/fp_tls13_alpn_http11_probe" >/dev/null
)
...
(
  cd "$ROOT_DIR"
  "$WORK_DIR/fp_tls13_alpn_http11_probe" >"$CLIENT_LOG" 2>&1
) || {
  echo "--- client log ---"
  cat "$CLIENT_LOG" || true
  echo "--- server log (tail) ---"
  tail -n 120 "$SERVER_LOG" || true
  fail "pure Pascal TLS1.3 ALPN http/1.1 fallback probe should succeed"
}

rg -F --quiet -- "STREAM_OK" "$CLIENT_LOG" || {
  cat "$CLIENT_LOG"
  fail "TLS1.3 ALPN http/1.1 fallback probe should report STREAM_OK"
}

rg -F --quiet -- "ALPN=http/1.1 INFO_ALPN=http/1.1" "$CLIENT_LOG" || {
  cat "$CLIENT_LOG"
  fail "TLS1.3 ALPN fallback probe should expose negotiated http/1.1 on both APIs"
}

echo "[PASS] pure Pascal TLS1.3 ALPN http/1.1 fallback contract passed"

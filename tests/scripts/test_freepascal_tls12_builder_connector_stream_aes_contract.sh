...
WORK_DIR="$(mktemp -d -t fafafa_fp_tls12_builder_aes_XXXXXX)"
PORT=18474
...
echo "[TEST] pure Pascal TLS1.2 builder/connector/stream AES contract"
...
cat > "$WORK_DIR/fp_tls12_builder_aes_probe.pas" <<'PAS'
program fp_tls12_builder_aes_probe;
...
begin
  ...
  try
    ...
    WriteLn(
      'STREAM_AES_OK PROTO=', Ord(Stream.Connection.GetProtocolVersion),
      ' CIPHER=', Stream.Connection.GetCipherName,
...
(
  ...
  fpc -B -Mobjfpc -Sh \
    ...
    "$WORK_DIR/fp_tls12_builder_aes_probe.pas" \
    -o"$WORK_DIR/fp_tls12_builder_aes_probe" >/dev/null
)
...
(
  cd "$ROOT_DIR"
  "$WORK_DIR/fp_tls12_builder_aes_probe" >"$CLIENT_LOG" 2>&1
) || {
  echo "--- client log ---"
  cat "$CLIENT_LOG" || true
  echo "--- server log (tail) ---"
  tail -n 120 "$SERVER_LOG" || true
  fail "pure Pascal TLS1.2 builder/connector/stream AES probe should succeed"
}

rg -F --quiet -- "STREAM_AES_OK" "$CLIENT_LOG" || {
  cat "$CLIENT_LOG"
  fail "builder/connector/stream AES probe should report STREAM_AES_OK"
}

rg -F --quiet -- "CIPHER=TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256" "$CLIENT_LOG" || {
  cat "$CLIENT_LOG"
  fail "builder/connector/stream AES probe should report AES128-GCM cipher"
}

echo "[PASS] pure Pascal TLS1.2 builder/connector/stream AES contract passed"

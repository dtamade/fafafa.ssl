...
require_fixed "$facade" "TSSLOption = fafafa.ssl.base.TSSLOption;" \
  "main facade must re-export TSSLOption"
require_fixed "$facade" "TSSLOptions = fafafa.ssl.base.TSSLOptions;" \
  "main facade must re-export TSSLOptions"
require_fixed "$facade" "ssoEnableSNI = fafafa.ssl.base.ssoEnableSNI;" \
  "main facade must re-export ssoEnableSNI"
require_fixed "$facade" "ssoEnableALPN = fafafa.ssl.base.ssoEnableALPN;" \
  "main facade must re-export ssoEnableALPN"
require_fixed "$facade" "ssoRequireCertificateTransparency = fafafa.ssl.base.ssoRequireCertificateTransparency;" \
  "main facade must re-export the tail option constants too"
require_fixed "$api_ref" '主门面 `fafafa.ssl` 当前也 re-export `TSSLOption` / `TSSLOptions` 与 `sso*` option 常量；普通调用方配置 context options 时不需要回退 `fafafa.ssl.base`。' \

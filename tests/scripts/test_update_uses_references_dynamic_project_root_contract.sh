...
SCRIPT="$ROOT_DIR/scripts/update_uses_references.ps1"
...
echo "[TEST] update_uses_references dynamic project root contract"
...
if ! rg -F --quiet -- '$srcDir = $resolvedProjectRoot' "$SCRIPT"; then
  fail "script should derive scan root from resolved project root"
fi
...
echo "[PASS] update_uses_references dynamic project root contract passed"

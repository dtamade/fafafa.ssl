...
require_fixed "$user_guide" '普通跨后端新代码优先使用 builder / default-context 的 shipped baseline；只有在 `SupportsCustomCipherSuites=True` 的 backend（当前主要是 OpenSSL）上，才传入 custom non-default cipher override。' \
...
require_fixed "$troubleshooting" '如果你明确锁定的是 `SupportsCustomCipherSuites=True` 的 backend（当前主要是 OpenSSL），才继续调 custom cipher；否则优先保留 shipped baseline defaults。' \
...
require_fixed "$security_best" '只有在 `ISSLLibrary.GetCapabilities.SupportsCustomCipherSuites=True` 的 backend（当前主要是 OpenSSL）上，才追加 `WithCipherList(...)` / `WithTLS13Ciphersuites(...)`。' \
...
require_fixed "$readme_file" '如果你明确锁定的是支持 custom cipher override 的 backend（当前主要是 OpenSSL），再在 capability check 之后调用 `SetCipherList(...)` / `SetCipherSuites(...)`。' \
...
require_fixed "$api_documentation" '如需 custom cipher allowlist，请只在 `SupportsCustomCipherSuites=True` 的 backend（当前主要是 OpenSSL）上追加这类 builder 配置。' \

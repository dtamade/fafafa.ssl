...
WORK_REL="tmp/test_prepare_wave_b_b2_handoff_bundle_report_chain_$(date +%s)"
WORK_DIR="$ROOT_DIR/$WORK_REL"
FAKE_ROOT="$WORK_DIR/fake_project"
FAKE_SCRIPTS="$FAKE_ROOT/scripts"
RUN_ID="handoff_report_chain_truth"
...
if ! rg -n "^- handoff_state: \\*\\*NEEDS_REPORT_REPAIR\\*\\*$" "$REPORT_ABS" >/dev/null; then
  fail "handoff bundle should surface malformed downstream report metadata as NEEDS_REPORT_REPAIR instead of pretending the handoff can continue"
fi

if ! rg -n "report_chain_note: .*consistency_status missing" "$REPORT_ABS" >/dev/null; then
  fail "handoff bundle should record which downstream report metadata is malformed"
fi

if rg -n "^- handoff_state: \\*\\*READY_FOR_RUNNER\\*\\*$|^- handoff_state: \\*\\*CLOSED\\*\\*$" "$REPORT_ABS" >/dev/null; then
  fail "handoff bundle must not silently downgrade malformed report-chain metadata into a normal runner or closed state"
fi

echo "[PASS] prepare_wave_b_b2 handoff bundle report-chain contract passed"

...
WORK_DIR="$(mktemp -d -t fafafa_fp_tls12_p256_XXXXXX)"
PORT=18481
...
echo "[TEST] pure Pascal TLS1.2 local P-256 contract"

require_cmd openssl
require_cmd fpc

cat > "$WORK_DIR/fp_tls12_p256_probe.pas" <<'PAS'
program fp_tls12_p256_probe;
...
var
  ...
  Sock: TSocketHandle;
begin
  Sock := ConnectTCP('127.0.0.1', 18481);
  ...
  WriteLn('P256_OK CIPHER=', Conn.GetCipherName);
end.
PAS
...
(
  ...
  fpc -B -Mobjfpc -Sh \
    ...
    "$WORK_DIR/fp_tls12_p256_probe.pas" \
    -o"$WORK_DIR/fp_tls12_p256_probe" >/dev/null
)
...
(
  cd "$ROOT_DIR"
  "$WORK_DIR/fp_tls12_p256_probe" >"$CLIENT_LOG" 2>&1
)
STATUS=$?
set -e
...
if [[ "$STATUS" -ne 0 ]]; then
  ...
  fail "pure Pascal TLS1.2 P-256 probe should succeed"
fi

rg -F --quiet -- "P256_OK" "$CLIENT_LOG" || {
  cat "$CLIENT_LOG"
  fail "P-256 probe should report P256_OK"
}

echo "[PASS] pure Pascal TLS1.2 local P-256 contract passed"

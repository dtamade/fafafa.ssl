...
WORK_REL="tmp/test_wave_b_cross_examples_static_fallback_$$"
WORK_DIR="$ROOT_DIR/$WORK_REL"
REPORTS_REL="$WORK_REL/reports"
REPORTS_DIR="$ROOT_DIR/$REPORTS_REL"
RUN_ID="wave_b_cross_static_fallback_$$"
...
echo "[TEST] wave-b cross-platform summary default examples static same-run fallback contract"
...
assert_contains "$OUTPUT_FILE" "- linux_examples_json: $REPORTS_REL/examples_compile_ci_gate.json"
assert_contains "$OUTPUT_FILE" '| total | 75 |'
assert_contains "$OUTPUT_FILE" '| pass_rate | 94.7% |'

echo "[PASS] wave-b cross-platform summary default examples static same-run fallback contract passed"

...
openssl_api_ssl="src/fafafa.ssl.openssl.api.ssl.pas"
...
require_fixed "$freepascal_lib" "Result.SupportsCustomCipherSuites := False;" \
  "FreePascal must not publish custom cipher capability before runtime support exists"
require_pcre "$freepascal_ctx" "procedure TFreePascalContext\\.SetCipherList\\(const ACipherList: string\\);.*?if IsCustomCipherListOverride\\(ACipherList\\) then\\s+RejectUnsupportedCustomCipherAssignment\\('Cipher list', 'TFreePascalContext\\.SetCipherList'\\);" \
  "FreePascal cipher-list setter must reject custom non-default overrides"
require_pcre "$freepascal_ctx" "procedure TFreePascalContext\\.SetCipherSuites\\(const ACipherSuites: string\\);.*?if IsCustomCipherSuitesOverride\\(ACipherSuites\\) then\\s+RejectUnsupportedCustomCipherAssignment\\('Cipher suites', 'TFreePascalContext\\.SetCipherSuites'\\);" \
  "FreePascal cipher-suites setter must reject custom non-default overrides"
...
require_fixed "$backend_matrix" "| **Custom Cipher Suites**      | ❌         | ✅      | ❌     | ❌      | ❌      |" \
...
require_fixed "$backend_matrix" '- `FreePascal` / `WinSSL` / `MbedTLS` / `WolfSSL`: `SupportsCustomCipherSuites=False`；custom non-default `SetCipherList` / `SetCipherSuites` 当前会 fail-closed，empty clear / shipped baseline defaults 仅保留 compatibility/default-context path' \
  "backend matrix must record false-backend fail-closed custom-cipher truth"

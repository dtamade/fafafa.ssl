...
if ! rg -F --quiet -- 'Get-ChildItem -Path $srcDir -Filter "fafafa.ssl.openssl*.pas" -File' "$SCRIPT"; then
  fail "validate_all_modules.ps1 should scan src/ via Get-ChildItem -Filter fafafa.ssl.openssl*.pas"
fi
...
if rg -F --quiet -- 'fafafa.ssl.openssl.core.pas' "$SCRIPT"; then
  fail "validate_all_modules.ps1 must not reference legacy module path fafafa.ssl.openssl.core.pas"
fi

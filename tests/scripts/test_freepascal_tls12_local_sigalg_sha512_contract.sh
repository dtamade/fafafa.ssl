...
WORK_DIR="$(mktemp -d -t fafafa_fp_tls12_sig512_XXXXXX)"
PORT=18475
...
echo "[TEST] pure Pascal TLS1.2 local rsa_pkcs1_sha512 contract"

require_cmd openssl
require_cmd fpc

cat > "$WORK_DIR/fp_tls12_sig512_probe.pas" <<'PAS'
program fp_tls12_sig512_probe;
...
var
  ...
  Sock: TSocketHandle;
begin
  Sock := ConnectTCP('127.0.0.1', 18475);
  ...
  WriteLn('SIG512_OK CIPHER=', Conn.GetCipherName);
end.
PAS
...
(
  ...
  fpc -B -Mobjfpc -Sh \
    ...
    "$WORK_DIR/fp_tls12_sig512_probe.pas" \
    -o"$WORK_DIR/fp_tls12_sig512_probe" >/dev/null
)
...
(
  cd "$ROOT_DIR"
  "$WORK_DIR/fp_tls12_sig512_probe" >"$CLIENT_LOG" 2>&1
)
STATUS=$?
set -e
...
if [[ "$STATUS" -ne 0 ]]; then
  ...
  fail "pure Pascal TLS1.2 rsa_pkcs1_sha512 probe should succeed"
fi

rg -F --quiet -- "SIG512_OK" "$CLIENT_LOG" || {
  cat "$CLIENT_LOG"
  fail "rsa_pkcs1_sha512 probe should report SIG512_OK"
}

echo "[PASS] pure Pascal TLS1.2 local rsa_pkcs1_sha512 contract passed"

...
advanced_unit="src/fafafa.ssl.cert.advanced.pas"
pem_unit="src/fafafa.ssl.openssl.api.pem.pas"

echo "[TEST] PKCS12 helper guide active truth contract"

require_fixed "$facade" "TPKCS12Manager = fafafa.ssl.cert.advanced.TPKCS12Manager;" \
...
require_fixed "$guide" '- OpenSSL raw API：`fafafa.ssl.openssl.api.pkcs12` + `fafafa.ssl.openssl.api.pem`' \

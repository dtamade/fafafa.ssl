program test_openssl_connection_stream_read_contract;
...
procedure AssertStreamReadSafeDegrade(const AName: string; AContext: ISSLContext);
var
  LStream: TMemoryStream;
  LConn: TOpenSSLConnectionAccess;
...
procedure TestStreamReadShouldFailGracefullyWhenHelpersAreUnavailable;
var
...
begin
  WriteLn;
  WriteLn('=== OpenSSL connection stream read guard ===');
  ...
  begin
    MarkSkip('openssl connection stream read contract',
      'required baseline OpenSSL stream read helpers are unavailable');
    Exit;
  end;
  ...
  try
    ...
    AssertStreamReadSafeDegrade(
      'Read baseline on stream transport when SSL_read fails',
      LContext
    );
    ...
    AssertStreamReadSafeDegrade(
      'Read on stream transport when SSL_read is unavailable',
      LContext
    );
    ...
    SSL_get_error := nil;
    AssertStreamReadSafeDegrade(
      'Read on stream transport when SSL_get_error is unavailable on failure path',
      LContext
    );
  finally
...
begin
  WriteLn('========================================');
  WriteLn('OpenSSL Connection Stream Read Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestStreamReadShouldFailGracefullyWhenHelpersAreUnavailable;

program test_cert_utils_generate_signed_x509_get_serialnumber_symbol_contract;
...
begin
  ...
  Result.CommonName := 'signed-x509-get-serial-root.local';
...
begin
  Result := TCertificateUtils.DefaultGenOptions;
  Result.CommonName := 'signed-x509-get-serial-leaf.local';
  Result.Organization := 'fafafa.ssl contract';
  Result.IsCA := False;
  Result.ValidDays := 30;
end;
...
procedure TestGenerateSignedShouldFailGracefullyWhenX509GetSerialNumberIsUnavailable;
var
  LLeafOptions: TCertGenOptions;
  LCACertPEM: string;
  LCAKeyPEM: string;
...
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSigned X509_get_serialNumber symbol guard ===');

  if not Assigned(X509_get_serialNumber) then
  begin
    MarkSkip('certificate utils generate-signed X509_get_serialNumber contract',
      'required baseline X509_get_serialNumber helper is unavailable');
    Exit;
  end;
  ...
  try
    AssertGenerateSignedControlledFailure(
      'GenerateSigned when X509_get_serialNumber is unavailable',
      LLeafOptions,
      LCACertPEM,
      LCAKeyPEM
    );
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSigned X509_get_serialNumber Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSignedShouldFailGracefullyWhenX509GetSerialNumberIsUnavailable;

program test_cert_utils_generate_selfsigned_certificate_pem_export_bio_free_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-certificate-export-bio-free-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenCertificateExportBIOFreeBecomesUnavailable;
var
  LOptions: TCertGenOptions;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSelfSigned certificate PEM export BIO_free symbol guard ===');

  if (not Assigned(BIO_read)) or (not Assigned(BIO_free)) then
  begin
    MarkSkip('certificate utils GenerateSelfSigned certificate PEM export BIO_free contract',
  ...
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when certificate PEM export BIO_free becomes unavailable',
      LOptions
    );
  finally
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned Certificate PEM Export BIO_free Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenCertificateExportBIOFreeBecomesUnavailable;

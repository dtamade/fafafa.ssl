program test_cert_utils_generate_selfsigned_x509_get_serialnumber_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-x509-get-serial-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenX509GetSerialNumberIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalX509GetSerialNumber: TX509_get_serialNumber;
begin
  WriteLn;
  WriteLn('=== Certificate utils self-signed X509_get_serialNumber symbol guard ===');

  if not Assigned(X509_get_serialNumber) then
  begin
    MarkSkip('certificate utils selfsigned X509_get_serialNumber contract',
  ...
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when X509_get_serialNumber is unavailable',
      LOptions
    );
  finally
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned X509_get_serialNumber Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenX509GetSerialNumberIsUnavailable;

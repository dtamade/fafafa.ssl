program test_cert_utils_getinfo_x509_get_notafter_symbol_contract;
...
procedure AssertNotAfterGuardInfo(
  const AName: string;
...
procedure TestGetInfoShouldFailGracefullyWhenX509GetNotAfterIsUnavailable;
var
  LFixturePEM: string;
...
begin
  WriteLn;
  WriteLn('=== Certificate utils GetInfo X509_get_notAfter symbol guard ===');
  ...
  if (not Assigned(BIO_new_mem_buf)) or
     ...
     (not Assigned(X509_get_notAfter)) then
  begin
    MarkSkip('certificate utils getinfo X509_get_notAfter symbol contract',
  ...
  try
    AssertGetInfoSafeDegrade(
      'GetInfo when X509_get_notAfter is unavailable',
      LFixturePEM,
      LExpectedSubject,
      LExpectedIssuer,
      LExpectedVersion,
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GetInfo X509_get_notAfter Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGetInfoShouldFailGracefullyWhenX509GetNotAfterIsUnavailable;

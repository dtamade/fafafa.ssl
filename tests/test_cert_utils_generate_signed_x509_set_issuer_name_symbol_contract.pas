program test_cert_utils_generate_signed_x509_set_issuer_name_symbol_contract;
...
begin
  ...
  Result.CommonName := 'signed-x509-set-issuer-name-root.local';
...
begin
  Result := TCertificateUtils.DefaultGenOptions;
  Result.CommonName := 'signed-x509-set-issuer-name-leaf.local';
  Result.Organization := 'fafafa.ssl contract';
  Result.IsCA := False;
  Result.ValidDays := 30;
end;
...
procedure TestGenerateSignedShouldFailGracefullyWhenX509SetIssuerNameIsUnavailable;
var
  LLeafOptions: TCertGenOptions;
  LCACertPEM: string;
  LCAKeyPEM: string;
  LOriginalX509SetIssuerName: TX509_set_issuer_name;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSigned X509_set_issuer_name symbol guard ===');

  if not Assigned(X509_set_issuer_name) then
  begin
    MarkSkip('certificate utils generate-signed X509_set_issuer_name contract',
      'required baseline X509_set_issuer_name helper is unavailable');
    Exit;
  end;
  ...
  LOriginalX509SetIssuerName := X509_set_issuer_name;
  X509_set_issuer_name := nil;
  try
    AssertGenerateSignedControlledFailure(
      'GenerateSigned when X509_set_issuer_name is unavailable',
      LLeafOptions,
      LCACertPEM,
      LCAKeyPEM
    );
  finally
    X509_set_issuer_name := LOriginalX509SetIssuerName;
  end;
end;

begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSigned X509_set_issuer_name Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSignedShouldFailGracefullyWhenX509SetIssuerNameIsUnavailable;

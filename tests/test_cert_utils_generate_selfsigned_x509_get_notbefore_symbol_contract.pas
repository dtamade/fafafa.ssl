program test_cert_utils_generate_selfsigned_x509_get_notbefore_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-x509-get-notbefore-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenX509GetNotBeforeIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalX509GetNotBefore: TX509_get_notBefore;
begin
  WriteLn;
  WriteLn('=== Certificate utils self-signed X509_get_notBefore symbol guard ===');

  if not Assigned(X509_get_notBefore) then
  begin
    MarkSkip('certificate utils selfsigned X509_get_notBefore contract',
  ...
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when X509_get_notBefore is unavailable',
      LOptions
    );
  finally
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned X509_get_notBefore Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenX509GetNotBeforeIsUnavailable;

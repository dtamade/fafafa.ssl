program test_cert_utils_generate_selfsigned_x509_set_pubkey_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-x509-set-pubkey-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenX509SetPubkeyIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalX509SetPubkey: TX509_set_pubkey;
begin
  WriteLn;
  WriteLn('=== Certificate utils self-signed X509_set_pubkey symbol guard ===');

  if not Assigned(X509_set_pubkey) then
  begin
    MarkSkip('certificate utils selfsigned X509_set_pubkey contract',
  ...
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when X509_set_pubkey is unavailable',
      LOptions
    );
  finally
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned X509_set_pubkey Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenX509SetPubkeyIsUnavailable;

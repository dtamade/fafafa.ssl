...
var
  ...
  GStoreCtxChain: fafafa.ssl.openssl.base.PSTACK_OF_X509 = nil;
...
function StubSkX509NewNull: fafafa.ssl.openssl.api.stack.PSTACK_OF_X509; cdecl;
begin
  if Assigned(OPENSSL_sk_new_null) then
    Result := fafafa.ssl.openssl.api.stack.PSTACK_OF_X509(OPENSSL_sk_new_null())
  else
    Result := nil;
end;

function StubSkX509Push(
  st: fafafa.ssl.openssl.api.stack.PSTACK_OF_X509;
  val: PX509
): Integer; cdecl;
begin
  if Assigned(OPENSSL_sk_push) then
    Result := OPENSSL_sk_push(fafafa.ssl.openssl.api.stack.POPENSSL_STACK(st), val)
  else
    Result := 0;
end;

function StubSkX509Num(
  const st: fafafa.ssl.openssl.api.stack.PSTACK_OF_X509
): Integer; cdecl;
begin
  if Assigned(OPENSSL_sk_num) then
    Result := OPENSSL_sk_num(fafafa.ssl.openssl.api.stack.POPENSSL_STACK(st))
  else
    Result := 0;
end;
...
begin
  if Assigned(OPENSSL_sk_value) then
    Result := PX509(OPENSSL_sk_value(fafafa.ssl.openssl.api.stack.POPENSSL_STACK(st), i))
  else
    Result := nil;
end;

procedure StubSkX509Free(st: fafafa.ssl.openssl.api.stack.PSTACK_OF_X509); cdecl;
begin
  if Assigned(OPENSSL_sk_free) then
    OPENSSL_sk_free(fafafa.ssl.openssl.api.stack.POPENSSL_STACK(st));
end;

function StubOCSPBasicRespVerify(
  bs: POCSP_BASICRESP;
  certs: fafafa.ssl.openssl.api.stack.PSTACK_OF_X509;
  st: PX509_STORE;
  flags: Cardinal
): Integer; cdecl;
begin
...
function StubX509StoreCtxInit(
  ...
  target: PX509;
  untrusted: fafafa.ssl.openssl.base.PSTACK_OF_X509
): Integer; cdecl;
begin
  Result := 1;
end;
...
function StubX509StoreCtxGet0Chain(
  ctx: PX509_STORE_CTX
): fafafa.ssl.openssl.base.PSTACK_OF_X509; cdecl;
begin
...
begin
  ...
  GStoreCtxChain := fafafa.ssl.openssl.base.PSTACK_OF_X509(sk_X509_new_null());
  if GStoreCtxChain = nil then
    raise Exception.Create('failed to allocate X509 stack for OCSP ownership contract');

  if sk_X509_push(
       fafafa.ssl.openssl.api.stack.PSTACK_OF_X509(GStoreCtxChain),
       GIssuerX509
     ) <> 1 then
...
begin
  ...
  begin
    if Assigned(sk_X509_free) then
      sk_X509_free(fafafa.ssl.openssl.api.stack.PSTACK_OF_X509(GStoreCtxChain));
    GStoreCtxChain := nil;
  end;

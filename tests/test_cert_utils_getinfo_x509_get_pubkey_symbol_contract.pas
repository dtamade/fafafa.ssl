program test_cert_utils_getinfo_x509_get_pubkey_symbol_contract;
...
procedure AssertPubKeyGuardInfo(
  const AName: string;
...
begin
  ...
  AssertTrue(AName + ' should keep public key bits at default', AInfo.PublicKeyBits = 0,
    'PublicKeyBits=' + IntToStr(AInfo.PublicKeyBits));
  AssertTrue(AName + ' should keep IsCA at default', not AInfo.IsCA,
    'IsCA=True');
...
procedure TestGetInfoShouldFailGracefullyWhenX509GetPubKeyIsUnavailable;
var
  LFixturePEM: string;
  ...
  LOriginalX509GetPubKey: TX509_get_pubkey;
begin
  WriteLn;
  WriteLn('=== Certificate utils GetInfo X509_get_pubkey symbol guard ===');
  ...
  if (not Assigned(BIO_new_mem_buf)) or
     ...
     (not Assigned(X509_get_pubkey)) then
  begin
    MarkSkip('certificate utils getinfo X509_get_pubkey symbol contract',
  ...
  LOriginalX509GetPubKey := X509_get_pubkey;
  X509_get_pubkey := nil;
  try
    AssertGetInfoSafeDegrade(
      'GetInfo when X509_get_pubkey is unavailable',
      LFixturePEM,
      LExpectedSubject,
      LExpectedIssuer,
      LExpectedVersion,
    ...
    X509_get_pubkey := LOriginalX509GetPubKey;
  end;
end;

begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GetInfo X509_get_pubkey Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGetInfoShouldFailGracefullyWhenX509GetPubKeyIsUnavailable;

program test_cert_utils_generate_selfsigned_asn1_integer_set_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-asn1-integer-set-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenASN1IntegerSetIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalASN1IntegerSet: TASN1_INTEGER_set;
begin
  WriteLn;
  WriteLn('=== Certificate utils self-signed ASN1_INTEGER_set symbol guard ===');

  if not Assigned(ASN1_INTEGER_set) then
  begin
    MarkSkip('certificate utils selfsigned ASN1_INTEGER_set contract',
  ...
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when ASN1_INTEGER_set is unavailable',
      LOptions
    );
  finally
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned ASN1_INTEGER_set Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenASN1IntegerSetIsUnavailable;

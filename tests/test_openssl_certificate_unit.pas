{**
 * Test: OpenSSL Certificate Unit Tests
 * Purpose: High-coverage unit tests for TOpenSSLCertificate and TOpenSSLCertificateStore
 *
 * Test Categories:
...
program test_openssl_certificate_unit;
...
begin
  ...
  if FileExists(TEST_CERT_FILE) and GLibraryAvailable then
  begin
    try
      LCert := TOpenSSLCertificate.Create(nil, False);
    ...
    Skip('LoadFromFile with valid CA bundle', 'CA bundle not found or library not available');
...
begin
  ...
  // Test round-trip if CA bundle exists and library available
  if FileExists(TEST_CERT_FILE) and GLibraryAvailable then
  begin
    try
      LCert := TOpenSSLCertificate.Create(nil, False);
      ...
      begin
        Test('SaveToPEM callable after LoadFromFile', True);
        LCert.SaveToPEM;
      end
      else
      ...
      on E: Exception do
        Skip('SaveToPEM round-trip', E.Message);
    end;
  end
  else
    Skip('SaveToPEM round-trip', 'CA bundle not found or library not available');
end;
...
begin
  ...
  begin
    ...
    Skip('Certificate info after load', 'CA bundle not found or library not available');
end;
...
begin
  ...
  try
    LCert := TOpenSSLCertificate.Create(nil, False);
    LStore := TOpenSSLCertificateStore.Create;
  ...
  begin
    ...
    Skip('Certificate verification after load', 'CA bundle not found or library not available');
end;
  ...
  begin
    try
      ...
      begin
        ...
        LCert.GetFingerprintSHA1;
        Test('GetFingerprintSHA256 callable after load', True);
        LCert.GetFingerprintSHA256;
      end
      else
    ...
    Skip('Fingerprint after load', 'CA bundle not found or library not available');
end;
...
begin
  ...
  try
    LCert := TOpenSSLCertificate.Create(nil, False);
    LIssuer := TOpenSSLCertificate.Create(nil, False);
...
begin
  ...
  begin
    ...
    Skip('Store operations with CA bundle', 'CA bundle not found or library not available');

  // Test LoadFromPath with valid directory
  if DirectoryExists(TEST_CERT_DIR) and GLibraryAvailable then
  begin
    try
      LStore := TOpenSSLCertificateStore.Create;
      Test('LoadFromPath with valid dir callable', True);
    ...
    Skip('LoadFromPath with valid dir', 'Cert directory not found or library not available');
end;
...
begin
  WriteLn('OpenSSL Certificate Unit Tests');
  WriteLn('===============================');
  WriteLn('High-coverage tests for TOpenSSLCertificate and TOpenSSLCertificateStore');
  WriteLn('');
  ...
  try
    ...
    if GLibraryAvailable then
      ...
      WriteLn('OpenSSL library: Not available (tests will use empty certificates)');
  except
    on E: Exception do
    begin
  ...
  WriteLn('');
  WriteLn('========================================');
  WriteLn('OpenSSL Certificate Unit Test Summary');
  WriteLn('========================================');

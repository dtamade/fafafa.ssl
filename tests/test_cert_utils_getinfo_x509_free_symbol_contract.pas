program test_cert_utils_getinfo_x509_free_symbol_contract;
...
procedure AssertX509FreeGuardInfo(
  const AName: string;
...
procedure TestGetInfoShouldFailGracefullyWhenX509FreeIsUnavailable;
var
  LFixturePEM: string;
...
begin
  WriteLn;
  WriteLn('=== Certificate utils GetInfo X509_free symbol guard ===');
  ...
  begin
    MarkSkip('certificate utils getinfo X509_free symbol contract',
  ...
  try
    ...
    try
      AssertGetInfoSafeDegrade(
        'GetInfo when X509_free is unavailable',
        LFixturePEM,
        LExpectedSubject,
        LExpectedIssuer,
        LExpectedVersion,
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GetInfo X509_free Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGetInfoShouldFailGracefullyWhenX509FreeIsUnavailable;

program test_cert_utils_getinfo_x509_get_version_symbol_contract;
...
procedure AssertVersionGuardInfo(
  const AName: string;
...
begin
  ...
  begin
    try
      AssertVersionGuardInfo(AName, LInfo, AExpectedSubject, AExpectedIssuer);
    finally
      if Assigned(LInfo.SubjectAltNames) then
        LInfo.SubjectAltNames.Free;
    end;
  ...
  begin
    try
      AssertVersionGuardInfo(AName + ' Try wrapper', LTryInfo, AExpectedSubject, AExpectedIssuer);
...
procedure TestGetInfoShouldFailGracefullyWhenX509GetVersionIsUnavailable;
var
  LFixturePEM: string;
  LExpectedSubject: string;
  LExpectedIssuer: string;
  LOriginalX509GetVersion: TX509_get_version;
begin
  WriteLn;
  WriteLn('=== Certificate utils GetInfo X509_get_version symbol guard ===');
  ...
  if (not Assigned(BIO_new_mem_buf)) or
     ...
     (not Assigned(X509_get_version)) then
  begin
    MarkSkip('certificate utils getinfo X509_get_version symbol contract',
  ...
  WarmupGetInfo(LFixturePEM, LExpectedSubject, LExpectedIssuer);

  LOriginalX509GetVersion := X509_get_version;
  X509_get_version := nil;
  try
    AssertGetInfoSafeDegrade(
      'GetInfo when X509_get_version is unavailable',
      LFixturePEM,
      LExpectedSubject,
      LExpectedIssuer
    );
  finally
    X509_get_version := LOriginalX509GetVersion;
  end;
end;

begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GetInfo X509_get_version Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGetInfoShouldFailGracefullyWhenX509GetVersionIsUnavailable;

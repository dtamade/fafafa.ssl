program test_cert_utils_generate_selfsigned_addextension_x509v3_ext_conf_nid_symbol_contract;
...
type
  TX509V3ExtConfNidFunc = function(conf: Pointer; ctx: PX509V3_CTX;
    ext_nid: Integer; const value: PAnsiChar): Pointer; cdecl;
...
begin
  ...
  Result.CommonName := 'selfsigned-x509v3-ext-conf-nid-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenX509V3ExtConfNidIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalX509V3ExtConfNid: TX509V3ExtConfNidFunc;
begin
  WriteLn;
  WriteLn('=== Certificate utils self-signed AddExtension X509V3_EXT_conf_nid symbol guard ===');

  if not Assigned(X509V3_EXT_conf_nid) then
  begin
    MarkSkip('certificate utils selfsigned AddExtension X509V3_EXT_conf_nid contract',
      'required baseline X509V3_EXT_conf_nid helper is unavailable');
    Exit;
  end;
  ...
  LOriginalX509V3ExtConfNid := X509V3_EXT_conf_nid;
  X509V3_EXT_conf_nid := nil;
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when X509V3_EXT_conf_nid is unavailable',
      LOptions
    );
  finally
    X509V3_EXT_conf_nid := LOriginalX509V3ExtConfNid;
  end;
end;

begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned AddExtension X509V3_EXT_conf_nid Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenX509V3ExtConfNidIsUnavailable;

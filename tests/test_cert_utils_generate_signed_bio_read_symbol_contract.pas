program test_cert_utils_generate_signed_bio_read_symbol_contract;
...
begin
  ...
  Result.CommonName := 'signed-bio-read-root.local';
...
begin
  Result := TCertificateUtils.DefaultGenOptions;
  Result.CommonName := 'signed-bio-read-leaf.local';
  Result.Organization := 'fafafa.ssl contract';
  Result.IsCA := False;
  Result.ValidDays := 30;
end;
...
procedure TestGenerateSignedShouldFailGracefullyWhenBIOReadIsUnavailable;
var
  LLeafOptions: TCertGenOptions;
  LCACertPEM: string;
  LCAKeyPEM: string;
...
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSigned BIO_read symbol guard ===');

  if not Assigned(BIO_read) then
  begin
    MarkSkip('certificate utils generate-signed BIO_read contract',
  ...
  try
    AssertGenerateSignedControlledFailure(
      'GenerateSigned when BIO_read is unavailable',
      LLeafOptions,
      LCACertPEM,
      LCAKeyPEM
    );
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSigned BIO_read Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSignedShouldFailGracefullyWhenBIOReadIsUnavailable;

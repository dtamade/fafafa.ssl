program test_winssl_hostname_mismatch_online;
...
begin
  ...
  try
    ...
    try
      ...
      Check('证书验证失败（期望主机名不匹配）', vr <> 0);
      if (vr = CERT_E_CN_NO_MATCH) or (vr = CERT_E_INVALID_NAME) then
        Check('错误码 CN_NO_MATCH/INVALID_NAME', True)
      else

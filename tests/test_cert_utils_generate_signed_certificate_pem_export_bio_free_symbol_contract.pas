program test_cert_utils_generate_signed_certificate_pem_export_bio_free_symbol_contract;
...
begin
  ...
  Result.CommonName := 'signed-certificate-export-bio-free-root.local';
...
begin
  Result := TCertificateUtils.DefaultGenOptions;
  Result.CommonName := 'signed-certificate-export-bio-free-leaf.local';
  Result.Organization := 'fafafa.ssl contract';
  Result.IsCA := False;
  Result.ValidDays := 30;
end;
...
function DisableBIOFreeAfterCertificatePEMRead(ABIO: Pointer; AData: Pointer; ADLen: Integer): Integer; cdecl;
begin
  Inc(GBIOReadCallCount);
  Result := GOriginalBIORead(ABIO, AData, ADLen);
  if (GBIOReadCallCount = 1) and (Result > 0) then
    BIO_free := nil;
end;

procedure InstallCertificateExportBIOFreeFailureWrapper;
begin
  GBIOReadCallCount := 0;
  BIO_free := GOriginalBIOFree;
  BIO_read := @DisableBIOFreeAfterCertificatePEMRead;
end;
...
var
  ...
  LTryResult: Boolean;
begin
  InstallCertificateExportBIOFreeFailureWrapper;
...
procedure TestGenerateSignedShouldFailGracefullyWhenCertificateExportBIOFreeBecomesUnavailable;
var
  LLeafOptions: TCertGenOptions;
  LCACertPEM: string;
  LCAKeyPEM: string;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSigned certificate PEM export BIO_free symbol guard ===');

  if (not Assigned(BIO_read)) or (not Assigned(BIO_free)) then
  begin
    MarkSkip('certificate utils generate-signed certificate PEM export BIO_free contract',
  ...
  try
    AssertGenerateSignedControlledFailure(
      'GenerateSigned when certificate PEM export BIO_free becomes unavailable',
      LLeafOptions,
      LCACertPEM,
      LCAKeyPEM
    );
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSigned Certificate PEM Export BIO_free Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSignedShouldFailGracefullyWhenCertificateExportBIOFreeBecomesUnavailable;

program test_cert_utils_generate_signed_x509_get_subject_name_symbol_contract;
...
begin
  ...
  Result.CommonName := 'signed-x509-get-subject-name-root.local';
...
begin
  Result := TCertificateUtils.DefaultGenOptions;
  Result.CommonName := 'signed-x509-get-subject-name-leaf.local';
  Result.Organization := 'fafafa.ssl contract';
  Result.IsCA := False;
  Result.ValidDays := 30;
end;
...
procedure TestGenerateSignedShouldFailGracefullyWhenX509GetSubjectNameIsUnavailable;
var
  LLeafOptions: TCertGenOptions;
  LCACertPEM: string;
  LCAKeyPEM: string;
  LOriginalX509GetSubjectName: TX509_get_subject_name;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSigned X509_get_subject_name symbol guard ===');

  if not Assigned(X509_get_subject_name) then
  begin
    MarkSkip('certificate utils generate-signed X509_get_subject_name contract',
  ...
  LOriginalX509GetSubjectName := X509_get_subject_name;
  X509_get_subject_name := nil;
  try
    AssertGenerateSignedControlledFailure(
      'GenerateSigned when X509_get_subject_name is unavailable',
      LLeafOptions,
      LCACertPEM,
      LCAKeyPEM
    );
  finally
    X509_get_subject_name := LOriginalX509GetSubjectName;
  end;
end;

begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSigned X509_get_subject_name Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSignedShouldFailGracefullyWhenX509GetSubjectNameIsUnavailable;

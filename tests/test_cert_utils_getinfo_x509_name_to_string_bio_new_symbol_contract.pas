program test_cert_utils_getinfo_x509_name_to_string_bio_new_symbol_contract;
...
var
  ...
  GOriginalX509GetSubjectName: TX509_get_subject_name = nil;
...
function StubX509GetSubjectNameDisableBIONew(const ACert: PX509): PX509_NAME; cdecl;
begin
  if Assigned(GOriginalX509GetSubjectName) then
    Result := GOriginalX509GetSubjectName(ACert)
  else
    Result := nil;
...
procedure AssertNameConversionBIONewGuardInfo(
  const AName: string;
...
begin
  AssertTrue(AName + ' should empty subject', AInfo.Subject = '',
    'Subject=' + AInfo.Subject);
  AssertTrue(AName + ' should empty issuer', AInfo.Issuer = '',
    'Issuer=' + AInfo.Issuer);
...
begin
  ...
  begin
    try
      AssertNameConversionBIONewGuardInfo(
        AName,
        LInfo,
        AExpectedVersion,
        AExpectedNotBefore,
...
procedure TestGetInfoShouldFailGracefullyWhenX509NameToStringBIONewIsUnavailable;
var
  LFixturePEM: string;
...
begin
  WriteLn;
  WriteLn('=== Certificate utils GetInfo X509NameToString BIO_new symbol guard ===');
  ...
  if (not Assigned(BIO_new_mem_buf)) or
     ...
     (not Assigned(X509_NAME_print_ex)) or
  ...
  begin
    MarkSkip('certificate utils getinfo X509NameToString BIO_new symbol contract',
  ...
  try
    ...
    AssertTrue('GetInfo warmup should keep non-empty subject', LExpectedSubject <> '',
      'Subject=' + LExpectedSubject);
    AssertTrue('GetInfo warmup should keep non-empty issuer', LExpectedIssuer <> '',
      'Issuer=' + LExpectedIssuer);

    GOriginalBIONew := BIO_new;
    GOriginalX509GetSubjectName := X509_get_subject_name;
    X509_get_subject_name := @StubX509GetSubjectNameDisableBIONew;
    try
      AssertGetInfoSafeDegrade(
        'GetInfo when X509NameToString BIO_new is unavailable',
        LFixturePEM,
        LExpectedVersion,
        LExpectedNotBefore,
        LExpectedNotAfter,
      ...
      BIO_new := GOriginalBIONew;
      X509_get_subject_name := GOriginalX509GetSubjectName;
      GOriginalBIONew := nil;
      GOriginalX509GetSubjectName := nil;
    end;
  finally
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GetInfo X509NameToString BIO_new Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGetInfoShouldFailGracefullyWhenX509NameToStringBIONewIsUnavailable;

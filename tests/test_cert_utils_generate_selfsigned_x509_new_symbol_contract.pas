program test_cert_utils_generate_selfsigned_x509_new_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-x509-new-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenX509NewIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalX509New: TX509_new;
begin
  WriteLn;
  WriteLn('=== Certificate utils self-signed X509_new symbol guard ===');

  if not Assigned(X509_new) then
  begin
    MarkSkip('certificate utils selfsigned X509_new contract',
  ...
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when X509_new is unavailable',
      LOptions
    );
  finally
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned X509_new Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenX509NewIsUnavailable;

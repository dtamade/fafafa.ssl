program test_cert_utils_generate_selfsigned_private_key_bio_new_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-private-key-bio-new-contract.local';
...
function DisableBIONewBeforePrivateKeyExportBIOConstructor: PBIO_METHOD; cdecl;
begin
...
begin
  ...
  BIO_s_mem := @DisableBIONewBeforePrivateKeyExportBIOConstructor;
end;
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenPrivateKeyExportBIONewBecomesUnavailable;
var
  LOptions: TCertGenOptions;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSelfSigned private-key BIO_new symbol guard ===');

  if (not Assigned(BIO_s_mem)) or (not Assigned(BIO_new)) then
  begin
    MarkSkip('certificate utils GenerateSelfSigned private-key BIO_new contract',
  ...
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when private-key BIO_new becomes unavailable',
      LOptions
    );
  finally
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned Private-Key BIO_new Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenPrivateKeyExportBIONewBecomesUnavailable;

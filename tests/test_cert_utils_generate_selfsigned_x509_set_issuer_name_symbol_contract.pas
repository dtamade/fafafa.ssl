program test_cert_utils_generate_selfsigned_x509_set_issuer_name_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-x509-set-issuer-name-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenX509SetIssuerNameIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalX509SetIssuerName: TX509_set_issuer_name;
begin
  WriteLn;
  WriteLn('=== Certificate utils self-signed X509_set_issuer_name symbol guard ===');

  if not Assigned(X509_set_issuer_name) then
  begin
    MarkSkip('certificate utils selfsigned X509_set_issuer_name contract',
  ...
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when X509_set_issuer_name is unavailable',
      LOptions
    );
  finally
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned X509_set_issuer_name Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenX509SetIssuerNameIsUnavailable;

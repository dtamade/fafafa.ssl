program test_cert_utils_generate_selfsigned_evp_pkey_new_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-evp-pkey-new-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenEVPPKeyNewIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalEVPPKeyNew: TEVP_PKEY_new;
begin
  WriteLn;
  WriteLn('=== Certificate utils self-signed EVP_PKEY_new symbol guard ===');

  if not Assigned(EVP_PKEY_new) then
  begin
    MarkSkip('certificate utils selfsigned EVP_PKEY_new contract',
      'required baseline EVP_PKEY_new helper is unavailable');
    Exit;
  end;
  ...
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when EVP_PKEY_new is unavailable',
      LOptions
    );
  finally
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned EVP_PKEY_new Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenEVPPKeyNewIsUnavailable;

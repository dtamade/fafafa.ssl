program test_cert_utils_getinfo_x509_name_to_string_bio_read_symbol_contract;
...
procedure AssertNameConversionReadGuardInfo(
  const AName: string;
...
begin
  ...
  begin
    try
      AssertNameConversionReadGuardInfo(
        AName,
        LInfo,
        AExpectedVersion,
        AExpectedNotBefore,
...
procedure TestGetInfoShouldFailGracefullyWhenX509NameToStringBIOReadIsUnavailable;
var
  LFixturePEM: string;
...
begin
  WriteLn;
  WriteLn('=== Certificate utils GetInfo X509NameToString BIO_read symbol guard ===');
  ...
  begin
    MarkSkip('certificate utils getinfo X509NameToString BIO_read symbol contract',
  ...
  try
    ...
    try
      AssertGetInfoSafeDegrade(
        'GetInfo when X509NameToString BIO_read is unavailable',
        LFixturePEM,
        LExpectedVersion,
        LExpectedNotBefore,
        LExpectedNotAfter,
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GetInfo X509NameToString BIO_read Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGetInfoShouldFailGracefullyWhenX509NameToStringBIOReadIsUnavailable;

program test_cert_utils_generate_signed_ca_private_key_bio_free_symbol_contract;
...
begin
  ...
  Result.CommonName := 'signed-ca-private-key-bio-free-root.local';
...
begin
  Result := TCertificateUtils.DefaultGenOptions;
  Result.CommonName := 'signed-ca-private-key-bio-free-leaf.local';
  Result.Organization := 'fafafa.ssl contract';
  Result.IsCA := False;
  Result.ValidDays := 30;
end;
...
function DisableBIOFreeAfterCAPrivateKeyRead(
  ABIO: PBIO;
  APKey: PPEVP_PKEY;
...
begin
  Result := GOriginalPEMReadPrivateKey(ABIO, APKey, APasswordCallback, AUserData);
  if Result <> nil then
    BIO_free := nil;
end;

procedure InstallCAPrivateKeyBIOFreeFailureWrapper;
begin
  BIO_free := GOriginalBIOFree;
  PEM_read_bio_PrivateKey := @DisableBIOFreeAfterCAPrivateKeyRead;
end;
...
var
  ...
  LTryResult: Boolean;
begin
  InstallCAPrivateKeyBIOFreeFailureWrapper;
...
procedure TestGenerateSignedShouldFailGracefullyWhenCAPrivateKeyBIOFreeBecomesUnavailable;
var
  LLeafOptions: TCertGenOptions;
  LCACertPEM: string;
  LCAKeyPEM: string;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSigned CA private-key BIO_free symbol guard ===');

  if (not Assigned(PEM_read_bio_PrivateKey)) or (not Assigned(BIO_free)) then
  begin
    MarkSkip('certificate utils generate-signed CA private-key BIO_free contract',
      'required baseline PEM_read_bio_PrivateKey/BIO_free helpers are unavailable');
    Exit;
  end;
  ...
  try
    AssertGenerateSignedControlledFailure(
      'GenerateSigned when CA private-key BIO_free becomes unavailable',
      LLeafOptions,
      LCACertPEM,
      LCAKeyPEM
    );
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSigned CA Private-Key BIO_free Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSignedShouldFailGracefullyWhenCAPrivateKeyBIOFreeBecomesUnavailable;

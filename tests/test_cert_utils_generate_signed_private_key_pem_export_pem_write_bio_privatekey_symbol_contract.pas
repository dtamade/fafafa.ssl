program test_cert_utils_generate_signed_private_key_pem_export_pem_write_bio_privatekey_symbol_contract;
...
var
  ...
  GOriginalPEMWriteBioPrivateKey: TPEM_write_bio_PrivateKey = nil;
...
begin
  ...
  Result.CommonName := 'signed-private-key-export-pem-write-root.local';
...
begin
  Result := TCertificateUtils.DefaultGenOptions;
  Result.CommonName := 'signed-private-key-export-pem-write-leaf.local';
  Result.Organization := 'fafafa.ssl contract';
  Result.IsCA := False;
  Result.ValidDays := 30;
end;
...
function DisablePrivateKeyPEMWriteAfterSignedPrivateKeyExportBIOConstructor(
  const AType: PBIO_METHOD
): PBIO; cdecl;
begin
  Inc(GBIONewCallCount);
  Result := GOriginalBIONew(AType);
  if (GBIONewCallCount = 2) and (Result <> nil) then
    PEM_write_bio_PrivateKey := nil;
end;

procedure InstallPrivateKeyExportPEMWriteFailureWrapper;
begin
  GBIONewCallCount := 0;
  PEM_write_bio_PrivateKey := GOriginalPEMWriteBioPrivateKey;
  BIO_new := @DisablePrivateKeyPEMWriteAfterSignedPrivateKeyExportBIOConstructor;
end;
...
var
  ...
  LTryResult: Boolean;
begin
  InstallPrivateKeyExportPEMWriteFailureWrapper;
...
procedure TestGenerateSignedShouldFailGracefullyWhenPrivateKeyExportPEMWriteBecomesUnavailable;
var
  LLeafOptions: TCertGenOptions;
  LCACertPEM: string;
  LCAKeyPEM: string;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSigned private-key PEM export PEM_write_bio_PrivateKey symbol guard ===');

  if (not Assigned(BIO_new)) or (not Assigned(PEM_write_bio_PrivateKey)) then
  begin
    MarkSkip('certificate utils generate-signed private-key PEM export PEM_write_bio_PrivateKey contract',
      'required baseline BIO_new/PEM_write_bio_PrivateKey helpers are unavailable');
    Exit;
  end;
  ...
  GOriginalPEMWriteBioPrivateKey := PEM_write_bio_PrivateKey;
  try
    AssertGenerateSignedControlledFailure(
      'GenerateSigned when private-key PEM export PEM_write_bio_PrivateKey becomes unavailable',
      LLeafOptions,
      LCACertPEM,
      LCAKeyPEM
    );
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSigned Private-Key PEM Export PEM_write_bio_PrivateKey Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSignedShouldFailGracefullyWhenPrivateKeyExportPEMWriteBecomesUnavailable;

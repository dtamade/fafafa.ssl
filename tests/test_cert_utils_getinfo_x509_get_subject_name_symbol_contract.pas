program test_cert_utils_getinfo_x509_get_subject_name_symbol_contract;
...
procedure TestGetInfoShouldFailGracefullyWhenX509GetSubjectNameIsUnavailable;
var
  LFixturePEM: string;
  LOriginalX509GetSubjectName: TX509_get_subject_name;
begin
  WriteLn;
  WriteLn('=== Certificate utils GetInfo X509_get_subject_name symbol guard ===');

  if (not Assigned(BIO_new_mem_buf)) or
     (not Assigned(BIO_free)) or
     (not Assigned(PEM_read_bio_X509)) or
     (not Assigned(X509_get_subject_name)) then
  begin
    MarkSkip('certificate utils getinfo X509_get_subject_name symbol contract',
  ...
  try
    AssertGetInfoSafeDegrade('GetInfo when X509_get_subject_name is unavailable', LFixturePEM);
  finally
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GetInfo X509_get_subject_name Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGetInfoShouldFailGracefullyWhenX509GetSubjectNameIsUnavailable;

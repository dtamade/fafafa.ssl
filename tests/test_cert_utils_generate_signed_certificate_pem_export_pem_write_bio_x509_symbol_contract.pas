program test_cert_utils_generate_signed_certificate_pem_export_pem_write_bio_x509_symbol_contract;
...
begin
  ...
  Result.CommonName := 'signed-certificate-export-pem-write-bio-x509-root.local';
...
begin
  Result := TCertificateUtils.DefaultGenOptions;
  Result.CommonName := 'signed-certificate-export-pem-write-bio-x509-leaf.local';
  Result.Organization := 'fafafa.ssl contract';
  Result.IsCA := False;
  Result.ValidDays := 30;
end;
...
function DisablePEMWriteX509AfterSignedCertificateExportBIOConstructor(const AType: PBIO_METHOD): PBIO; cdecl;
begin
...
begin
  ...
  BIO_new := @DisablePEMWriteX509AfterSignedCertificateExportBIOConstructor;
end;
...
procedure TestGenerateSignedShouldFailGracefullyWhenCertificateExportPEMWriteBecomesUnavailable;
var
  LLeafOptions: TCertGenOptions;
  LCACertPEM: string;
  LCAKeyPEM: string;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSigned certificate PEM export PEM_write_bio_X509 symbol guard ===');

  if (not Assigned(BIO_new)) or (not Assigned(PEM_write_bio_X509)) then
  begin
    MarkSkip('certificate utils generate-signed certificate PEM export PEM_write_bio_X509 contract',
  ...
  try
    AssertGenerateSignedControlledFailure(
      'GenerateSigned when certificate PEM export PEM_write_bio_X509 becomes unavailable',
      LLeafOptions,
      LCACertPEM,
      LCAKeyPEM
    );
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSigned Certificate PEM Export PEM_write_bio_X509 Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSignedShouldFailGracefullyWhenCertificateExportPEMWriteBecomesUnavailable;

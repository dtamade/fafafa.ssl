program test_cert_utils_generate_selfsigned_certificate_pem_export_bio_new_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-certificate-export-bio-new-contract.local';
...
function DisableBIONewBeforeCertificateExportBIOConstructor: PBIO_METHOD; cdecl;
begin
  Inc(GBIOSMemCallCount);
  Result := GOriginalBIOSMem();
  if (GBIOSMemCallCount = 1) and (Result <> nil) then
    BIO_new := nil;
end;

procedure InstallCertificateExportBIONewFailureWrapper;
begin
  GBIOSMemCallCount := 0;
  BIO_new := GOriginalBIONew;
  BIO_s_mem := @DisableBIONewBeforeCertificateExportBIOConstructor;
end;
...
begin
  InstallCertificateExportBIONewFailureWrapper;
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenCertificateExportBIONewBecomesUnavailable;
var
  LOptions: TCertGenOptions;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSelfSigned certificate PEM export BIO_new symbol guard ===');

  if (not Assigned(BIO_s_mem)) or (not Assigned(BIO_new)) then
  begin
    MarkSkip('certificate utils GenerateSelfSigned certificate PEM export BIO_new contract',
  ...
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when certificate PEM export BIO_new becomes unavailable',
      LOptions
    );
  finally
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned Certificate PEM Export BIO_new Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenCertificateExportBIONewBecomesUnavailable;

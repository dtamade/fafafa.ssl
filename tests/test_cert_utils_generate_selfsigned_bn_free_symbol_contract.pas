program test_cert_utils_generate_selfsigned_bn_free_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-bn-free-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenBNFreeIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalBNFree: TBN_free;
begin
  WriteLn;
  WriteLn('=== Certificate utils self-signed BN_free symbol guard ===');

  if not Assigned(BN_free) then
  begin
    MarkSkip('certificate utils selfsigned BN_free contract',
      'required baseline BN_free helper is unavailable');
    Exit;
  end;
  ...
  LOriginalBNFree := BN_free;
  BN_free := nil;
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when BN_free is unavailable',
      LOptions
    );
  finally
    BN_free := LOriginalBNFree;
  end;
end;

begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned BN_free Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenBNFreeIsUnavailable;

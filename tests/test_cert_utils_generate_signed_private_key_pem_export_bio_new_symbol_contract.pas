program test_cert_utils_generate_signed_private_key_pem_export_bio_new_symbol_contract;
...
begin
  ...
  Result.CommonName := 'signed-private-key-export-bio-new-root.local';
...
begin
  Result := TCertificateUtils.DefaultGenOptions;
  Result.CommonName := 'signed-private-key-export-bio-new-leaf.local';
  Result.Organization := 'fafafa.ssl contract';
  Result.IsCA := False;
  Result.ValidDays := 30;
end;
...
function DisableBIONewBeforeSignedPrivateKeyExportBIOConstructor: PBIO_METHOD; cdecl;
begin
  Inc(GBIOSMemCallCount);
  Result := GOriginalBIOSMem();
  if (GBIOSMemCallCount = 2) and (Result <> nil) then
    BIO_new := nil;
end;

procedure InstallPrivateKeyExportBIONewFailureWrapper;
begin
  GBIOSMemCallCount := 0;
  BIO_new := GOriginalBIONew;
  BIO_s_mem := @DisableBIONewBeforeSignedPrivateKeyExportBIOConstructor;
end;
...
var
  ...
  LTryResult: Boolean;
begin
  InstallPrivateKeyExportBIONewFailureWrapper;
...
procedure TestGenerateSignedShouldFailGracefullyWhenPrivateKeyExportBIONewBecomesUnavailable;
var
  LLeafOptions: TCertGenOptions;
  LCACertPEM: string;
  LCAKeyPEM: string;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSigned private-key PEM export BIO_new symbol guard ===');

  if (not Assigned(BIO_s_mem)) or (not Assigned(BIO_new)) then
  begin
    MarkSkip('certificate utils generate-signed private-key PEM export BIO_new contract',
      'required baseline BIO_s_mem/BIO_new helpers are unavailable');
    Exit;
  end;
  ...
  try
    AssertGenerateSignedControlledFailure(
      'GenerateSigned when private-key PEM export BIO_new becomes unavailable',
      LLeafOptions,
      LCACertPEM,
      LCAKeyPEM
    );
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSigned Private-Key PEM Export BIO_new Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSignedShouldFailGracefullyWhenPrivateKeyExportBIONewBecomesUnavailable;

program test_cert_utils_generate_selfsigned_rsa_new_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-rsa-new-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenRSANewIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalRSANew: TRSA_new;
begin
  WriteLn;
  WriteLn('=== Certificate utils self-signed RSA_new symbol guard ===');

  if not Assigned(RSA_new) then
  begin
    MarkSkip('certificate utils selfsigned RSA_new contract',
      'required baseline RSA_new helper is unavailable');
    Exit;
  end;
  ...
  LOriginalRSANew := RSA_new;
  RSA_new := nil;
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when RSA_new is unavailable',
      LOptions
    );
  finally
    RSA_new := LOriginalRSANew;
  end;
end;

begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned RSA_new Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenRSANewIsUnavailable;

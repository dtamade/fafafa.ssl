...
type
  TPKCSD2IPKCS8PrivKeyInfo = fafafa.ssl.openssl.api.pkcs.Td2i_PKCS8_PRIV_KEY_INFO;
  TPKCSD2IX509Sig = fafafa.ssl.openssl.api.pkcs.Td2i_X509_SIG;
  TPKCSEVPPKCS82PKEY = fafafa.ssl.openssl.api.pkcs.TEVP_PKCS82PKEY;
  TPKCS8Decrypt = fafafa.ssl.openssl.api.pkcs12.TPKCS8_decrypt;
  TRSAD2IPrivateKey = fafafa.ssl.openssl.api.rsa.Td2i_RSAPrivateKey;
...
var
  LPKey: PEVP_PKEY;
  LP8Info: fafafa.ssl.openssl.api.pkcs.PPKCS8_PRIV_KEY_INFO;
...
begin
  ...
  try
    if (not Assigned(fafafa.ssl.openssl.api.pkcs.EVP_PKEY2PKCS8)) or
       (not Assigned(fafafa.ssl.openssl.api.pkcs.i2d_PKCS8_PRIV_KEY_INFO)) or
       (not Assigned(fafafa.ssl.openssl.api.pkcs.PKCS8_PRIV_KEY_INFO_free)) then
      Exit;

    LP8Info := fafafa.ssl.openssl.api.pkcs.EVP_PKEY2PKCS8(LPKey);
    if LP8Info = nil then
      Exit;

    LEncodedLength := fafafa.ssl.openssl.api.pkcs.i2d_PKCS8_PRIV_KEY_INFO(LP8Info, nil);
    if LEncodedLength <= 0 then
      Exit;

    SetLength(ADER, LEncodedLength);
    LEncodedPtr := @ADER[0];
    if fafafa.ssl.openssl.api.pkcs.i2d_PKCS8_PRIV_KEY_INFO(LP8Info, @LEncodedPtr) <> LEncodedLength then
    begin
      SetLength(ADER, 0);
      Exit;
    end;
    ...
    if LP8Info <> nil then
      fafafa.ssl.openssl.api.pkcs.PKCS8_PRIV_KEY_INFO_free(LP8Info);
    if LPKey <> nil then
      EVP_PKEY_free(LPKey);
  end;
end;
...
var
  LPKey: PEVP_PKEY;
  LP8Info: fafafa.ssl.openssl.api.pkcs.PPKCS8_PRIV_KEY_INFO;
  LEncrypted: fafafa.ssl.openssl.api.pkcs.PX509_SIG;
  LPassA: AnsiString;
  LEncodedLength: Integer;
  LEncodedPtr: PByte;
begin
  ...
  try
    if (not Assigned(fafafa.ssl.openssl.api.pkcs.EVP_PKEY2PKCS8)) or
       (not Assigned(fafafa.ssl.openssl.api.pkcs.PKCS8_PRIV_KEY_INFO_free)) or
       (not Assigned(fafafa.ssl.openssl.api.pkcs.i2d_X509_SIG)) or
       (not Assigned(fafafa.ssl.openssl.api.pkcs.X509_SIG_free)) or
       (not Assigned(fafafa.ssl.openssl.api.pkcs12.PKCS8_encrypt)) then
      Exit;
    ...
    LEncrypted := fafafa.ssl.openssl.api.pkcs12.PKCS8_encrypt(
    ...
    LEncodedLength := fafafa.ssl.openssl.api.pkcs.i2d_X509_SIG(LEncrypted, nil);
    if LEncodedLength <= 0 then
      Exit;

    SetLength(ADER, LEncodedLength);
    LEncodedPtr := @ADER[0];
    if fafafa.ssl.openssl.api.pkcs.i2d_X509_SIG(LEncrypted, @LEncodedPtr) <> LEncodedLength then
    begin
      SetLength(ADER, 0);
      Exit;
    end;
    ...
    if LEncrypted <> nil then
      fafafa.ssl.openssl.api.pkcs.X509_SIG_free(LEncrypted);
    if LP8Info <> nil then
      fafafa.ssl.openssl.api.pkcs.PKCS8_PRIV_KEY_INFO_free(LP8Info);
    if LPKey <> nil then
      EVP_PKEY_free(LPKey);
...
begin
  ...
  LOriginalD2IX509Sig := fafafa.ssl.openssl.api.pkcs.d2i_X509_SIG;
  ...
  begin
    fafafa.ssl.openssl.api.pkcs.d2i_X509_SIG := nil;
    try
      AssertLoadPrivateKeyFileControlledFailure(
      ...
      fafafa.ssl.openssl.api.pkcs.d2i_X509_SIG := LOriginalD2IX509Sig;
    end;
  end;

  LOriginalPKCS8Decrypt := fafafa.ssl.openssl.api.pkcs12.PKCS8_decrypt;
  ...
  begin
    fafafa.ssl.openssl.api.pkcs12.PKCS8_decrypt := nil;
    try
      AssertLoadPrivateKeyStreamControlledFailure(
      ...
      fafafa.ssl.openssl.api.pkcs12.PKCS8_decrypt := LOriginalPKCS8Decrypt;
    end;
  end;

  LOriginalEVPPKCS82PKEY := fafafa.ssl.openssl.api.pkcs.EVP_PKCS82PKEY;
  ...
  begin
    fafafa.ssl.openssl.api.pkcs.EVP_PKCS82PKEY := nil;
    try
      AssertLoadPrivateKeyFileControlledFailure(
      ...
      fafafa.ssl.openssl.api.pkcs.EVP_PKCS82PKEY := LOriginalEVPPKCS82PKEY;
    end;
  end;

  LOriginalD2IPKCS8PrivKeyInfo := fafafa.ssl.openssl.api.pkcs.d2i_PKCS8_PRIV_KEY_INFO;
  ...
  begin
    fafafa.ssl.openssl.api.pkcs.d2i_PKCS8_PRIV_KEY_INFO := nil;
    try
      AssertLoadPrivateKeyStreamControlledFailure(
      ...
      fafafa.ssl.openssl.api.pkcs.d2i_PKCS8_PRIV_KEY_INFO := LOriginalD2IPKCS8PrivKeyInfo;
    end;
  end;

  LOriginalD2IRSAPrivateKey := fafafa.ssl.openssl.api.rsa.d2i_RSAPrivateKey;
  ...
  begin
    fafafa.ssl.openssl.api.rsa.d2i_RSAPrivateKey := nil;
    try
      AssertLoadPrivateKeyFileControlledFailure(
      ...
      fafafa.ssl.openssl.api.rsa.d2i_RSAPrivateKey := LOriginalD2IRSAPrivateKey;
    end;
  end;

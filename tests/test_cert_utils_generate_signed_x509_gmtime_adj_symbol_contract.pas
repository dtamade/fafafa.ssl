program test_cert_utils_generate_signed_x509_gmtime_adj_symbol_contract;
...
begin
  ...
  Result.CommonName := 'signed-x509-gmtime-adj-root.local';
...
begin
  Result := TCertificateUtils.DefaultGenOptions;
  Result.CommonName := 'signed-x509-gmtime-adj-leaf.local';
  Result.Organization := 'fafafa.ssl contract';
  Result.IsCA := False;
  Result.ValidDays := 30;
end;
...
procedure TestGenerateSignedShouldFailGracefullyWhenX509GmtimeAdjIsUnavailable;
var
  LLeafOptions: TCertGenOptions;
  LCACertPEM: string;
  LCAKeyPEM: string;
  LOriginalX509GmtimeAdj: TX509_gmtime_adj;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSigned X509_gmtime_adj symbol guard ===');

  if not Assigned(X509_gmtime_adj) then
  begin
    MarkSkip('certificate utils generate-signed X509_gmtime_adj contract',
      'required baseline X509_gmtime_adj helper is unavailable');
    Exit;
  end;
  ...
  LOriginalX509GmtimeAdj := X509_gmtime_adj;
  X509_gmtime_adj := nil;
  try
    AssertGenerateSignedControlledFailure(
      'GenerateSigned when X509_gmtime_adj is unavailable',
      LLeafOptions,
      LCACertPEM,
      LCAKeyPEM
    );
  finally
    X509_gmtime_adj := LOriginalX509GmtimeAdj;
  end;
end;

begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSigned X509_gmtime_adj Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSignedShouldFailGracefullyWhenX509GmtimeAdjIsUnavailable;

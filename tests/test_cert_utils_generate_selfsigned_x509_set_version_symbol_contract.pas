program test_cert_utils_generate_selfsigned_x509_set_version_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-x509-set-version-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenX509SetVersionIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalX509SetVersion: TX509_set_version;
begin
  WriteLn;
  WriteLn('=== Certificate utils self-signed X509_set_version symbol guard ===');

  if not Assigned(X509_set_version) then
  begin
    MarkSkip('certificate utils selfsigned X509_set_version contract',
  ...
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when X509_set_version is unavailable',
      LOptions
    );
  finally
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned X509_set_version Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenX509SetVersionIsUnavailable;

program test_cert_utils_generate_selfsigned_bn_new_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-bn-new-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenBNNewIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalBNNew: TBN_new;
begin
  WriteLn;
  WriteLn('=== Certificate utils self-signed BN_new symbol guard ===');

  if not Assigned(BN_new) then
  begin
    MarkSkip('certificate utils selfsigned BN_new contract',
      'required baseline BN_new helper is unavailable');
    Exit;
  end;
  ...
  LOriginalBNNew := BN_new;
  BN_new := nil;
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when BN_new is unavailable',
      LOptions
    );
  finally
    BN_new := LOriginalBNNew;
  end;
end;

begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned BN_new Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenBNNewIsUnavailable;

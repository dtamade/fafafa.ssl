program test_cert_utils_generate_selfsigned_evp_pkey_free_symbol_contract;
...
begin
  ...
  Result.CommonName := 'selfsigned-evp-pkey-free-contract.local';
...
procedure TestGenerateSelfSignedShouldFailGracefullyWhenEVPPKeyFreeIsUnavailable;
var
  LOptions: TCertGenOptions;
  LOriginalEVPPKeyFree: TEVP_PKEY_free;
begin
  WriteLn;
  WriteLn('=== Certificate utils GenerateSelfSigned EVP_PKEY_free symbol guard ===');

  if not Assigned(EVP_PKEY_free) then
  begin
    MarkSkip('certificate utils GenerateSelfSigned EVP_PKEY_free contract',
  ...
  try
    AssertGenerateSelfSignedControlledFailure(
      'GenerateSelfSigned when EVP_PKEY_free is unavailable',
      LOptions
    );
  finally
...
begin
  WriteLn('========================================');
  WriteLn('Certificate Utils GenerateSelfSigned EVP_PKEY_free Symbol Contract Test');
  WriteLn('========================================');
  ...
  try
    ...
    if SkippedTests = 0 then
      TestGenerateSelfSignedShouldFailGracefullyWhenEVPPKeyFreeIsUnavailable;

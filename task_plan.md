...
## Current Batch
1. 写 focused contract，证明当前 quick sprint bundle 的 `--run-id` 仍可从 `eval` 命令串中逃逸。
2. 最小修改脚本，把 `B107/B108/B109/B110` step 从 `eval "$cmd"` 切到 argv 执行，并移除内联 shell 条件拼接。
3. 复跑新合同、既有 unified contract、`B120 tmp reports` 邻近合同和脚本语法检查。
4. 更新 working-memory，并在 review 后提交。

## Status
- [completed] identified eval-based execution boundary in wave c quick sprint bundle
- [completed] added focused contract for quick sprint bundle run-id shell escape
- [completed] replaced quick sprint bundle eval execution with direct argv execution
- [completed] focused verification and review closeout

## Current Evidence
- 当前 quick sprint bundle 原先通过：
  - `run_step() -> eval "$cmd"`
  - `B107/B108/B109/B110` 四个 step 都消费字符串命令
  - B107 step 还把 `--require-full-gate` 通过 `$( [[ ... ]] && printf ... )` 内联 shell 拼接注入命令串
- focused contract 已锁住这条执行边界：
  - `tests/scripts/test_run_wave_c_quick_sprint_bundle_run_id_injection_contract.sh`
    - payload 不得执行
    - fake nested B107/B108/B109/B110 runner 必须收到完整 `run-id` 原始值
    - `--require-full-gate` 仍需透传给 B107
- 修复后验证结果：
...
## Current Batch
1. 写 focused contract，证明当前 B129 oncall 的 `--run-id` 仍可从 `eval` 命令串中逃逸。
2. 最小修改脚本，把 `B125/B126` step 从 `eval "$cmd"` 切到 argv 执行。
3. 复跑新合同、下游 `B142` tmp lookup 合同和脚本语法检查。
4. 更新 working-memory，并在 review 后提交。

## Status
- [completed] identified eval-based execution boundary in wave c b129 oncall
- [completed] added focused contract for b129 run-id shell escape
- [completed] replaced b129 step eval execution with direct argv execution
- [completed] focused verification and review closeout

## Current Evidence
- 当前 B129 oncall 仍通过：
  - `run_step() -> eval "$cmd"`
  - `B125/B126` 两个 step 都消费字符串命令
- 虽然上一批已经修掉了 B125 自己的执行面，但 B129 仍把同一个 `RUN_ID` 重新暴露在：
  - `--run-id ${RUN_ID}`
  - `--output ${bundle_report}` / `--output ${history_report}`
- focused contract 已锁住这条执行边界：
  - `tests/scripts/test_run_wave_c_local_guard_oncall_check_run_id_injection_contract.sh`
    - payload 不得执行
    - fake nested B125/B126 runner 必须收到完整 `run-id` 原始值
- 修复后验证结果：
...
# Task Plan - Wave C B125 Local-First Guard Bundle Shell Hardening

## Goal
收口 `scripts/run_wave_c_local_first_guard_bundle.sh` 的 `eval` / 字符串执行风险，避免 `run-id` 被当成 shell 语法执行。

## Current Batch
1. 写 focused contract，证明当前 B125 bundle 的 `--run-id` 仍可从 `eval` 命令串中逃逸。
2. 最小修改脚本，把 `B123/B124` step 从 `eval "$cmd"` 切到 argv 执行。
3. 复跑新合同、既有 tmp-default 合同和脚本语法检查。
4. 更新 working-memory，并在 review 后提交。

## Status
- [completed] identified eval-based execution boundary in wave c b125 bundle
- [completed] added focused contract for b125 run-id shell escape
- [completed] replaced b125 step eval execution with direct argv execution
- [completed] focused verification and review closeout

## Current Evidence
- 当前 B125 bundle 仍通过：
  - `run_step() -> eval "$cmd"`
  - `B123/B124` 两个 step 都消费字符串命令
- 这让 `RUN_ID` 同时暴露在：
  - `--run-id ${RUN_ID}`
  - `--output ${continuity_report}` / `--output ${drift_report}`
- 作为 Wave C local-guard 链的更底层 bundle，B125 一旦继续保留 `eval`，上层 `B129 oncall` / `ops pack` 仍会继承这条真实执行面。
- focused contract 已锁住这条执行边界：
  - `tests/scripts/test_run_wave_c_local_first_guard_bundle_run_id_injection_contract.sh`
    - payload 不得执行
    - fake nested B123/B124 runner 必须收到完整 `run-id` 原始值
- 修复后验证结果：
...
## Current Batch
1. 写两个 focused contracts，分别证明 `--modules` 与 `--openssl3-lib-dir` 当前仍可注入 shell 语法。
2. 最小修改脚本，把 `eval` / 字符串执行切到 argv / env 数组。
3. 复跑新合同和脚本语法检查。
4. 更新 working-memory，然后 review 并提交。

## Status
- [completed] identified shell injection risk in linux openssl matrix modules and openssl3-lib-dir paths
- [completed] wrote focused contracts for linux matrix modules and openssl3-lib-dir injection
- [completed] minimal argv/env execution alignment in linux openssl matrix producer
- [completed] focused verification and review closeout
...
- 一次性复现已经证明两条真实风险：
  ...
  - `--openssl3-lib-dir "<payload with quote break>"` 会从 `LD_LIBRARY_PATH=...` 字符串前缀逃逸，且脚本仍可 `exit 0`
- focused contracts 已锁住两条注入边界：
  ...
  - `tests/scripts/test_linux_openssl_matrix_openssl3_lib_dir_injection_contract.sh`
    - payload 不得执行
    - fake openssl 仍必须观察到完整 `LD_LIBRARY_PATH` 原始值
- 修复后验证结果：

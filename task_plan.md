...
- Migrate `tests/test_freepascal_client_ocsp_stapling_runtime.pas` required
  OCSP stapling fail-closed text checks from direct
...
- Inspect `tests/test_freepascal_client_online_ocsp_runtime.pas` next; it
  appears to be another failure-text runtime check and may be safe to migrate
  to `ISSLCertificateVerification`.

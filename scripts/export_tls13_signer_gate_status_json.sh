...
选项：
  ...
  --output FILE       输出 JSON 路径（默认 tmp/tls13_signer_gate_reports/tls13_signer_gate_status_<run_id>.json）
...
if [[ -z "$OUTPUT_JSON" ]]; then
  OUTPUT_JSON="$REPORTS_DIR/tls13_signer_gate_status_${RUN_ID}.json"
fi
...
if [[ -z "$SNAPSHOT_FILE" ]]; then
  SNAPSHOT_FILE="$(resolve_exact_artifact "$REPORTS_DIR/tls13_signer_gate_snapshot_${RUN_ID}.md")"
fi

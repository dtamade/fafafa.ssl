...
begin
  ...
  try
    ...
    if EVP_EncryptUpdate(ctx, @ciphertext[0], @len, @plaintext[0], 64) <> 1 then Exit;
    clen := len;
    if EVP_EncryptFinal_ex(ctx, @ciphertext[clen], @len) <> 1 then Exit;
    clen := clen + len;
...
begin
  ...
  try
    ...
    if EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, 16, @tag[0]) <> 1 then Exit;
...
begin
  ...
  if not LoadChaCha20Functions(GetCryptoLibHandle) then
  begin
    WriteLn('ChaCha20 functions not available');
    Halt(0);
  end;
  
  if not LoadEVPFunctions(GetCryptoLibHandle) then
  begin

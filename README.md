...
统一 SSL/TLS 加密库，为 FreePascal 提供多后端抽象实现。

## 后端支持

| 后端 | 平台 | 状态 | 优先级 |
|------|------|------|--------|
| OpenSSL | Linux/macOS/Windows | 生产就绪 | 100 |
| MbedTLS | 跨平台 | 生产就绪 | 175 |
| WolfSSL | 跨平台 | 生产就绪 | 150 |
| WinSSL (Schannel) | Windows | 生产就绪 | 200 |
| FreePascal (纯 Pascal) | 跨平台 | 实验性 (TLS 1.3) | 50 |
...
├── fafafa.ssl.freepascal.*      纯 Pascal TLS 1.3 后端

...
这些测试命令是当前可执行验证入口，但不要把固定的总测试数、通过率或历史输出文本当成当前 helper/API truth。
...
- 综合测试程序成功编译并运行
- PKCS#12 helper/API 的 create / parse / workflow assertions 全部通过
- 如果当前运行的摘要数字与旧截图不同，以当前结果为准
...
- 当前指南重点记录的是 PKCS#12 helper/raw API surface、测试入口与用法示例

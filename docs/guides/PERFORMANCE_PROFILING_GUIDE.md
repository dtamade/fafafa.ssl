...
Ctx := TSSLContextBuilder.Create
  .WithSessionCache(True)  // 当前 builder 只控制是否启用 session cache
  .BuildClient;
Ctx.SetSessionCacheSize(1000);  // 如需限制缓存条目数，当前走 context API
...
这里要特别区分两层职责：`TSSLContextBuilder.WithSessionCache(True/False)` 当前只负责启用/关闭 session cache；如果你要调缓存条目数，仍应在 build 后通过 `ISSLContext.SetSessionCacheSize(...)` 配置。

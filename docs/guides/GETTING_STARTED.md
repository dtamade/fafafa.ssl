...
- v1.x 兼容路径对应字段：`TSSLConfig.UseSystemRoots`（或 `TSSLContextConfig.UseSystemRoots`）。
...
**v1.x 兼容路径**：如果你维护的旧代码不用 builder，仍可走 `TSSLConfig` + `TSSLFactory.CreateContext(...)`。
这是 v1.x 兼容入口，新代码推荐使用上面的 builder 路径。

...
- 当前 repo-side implementation bridge 已经完成安全收口：
  ...
  - `TWinSSLContext` 现在会在 server-side disable truth 下使用 Schannel `SCH_CRED_DISABLE_RECONNECTS`，且相关设置变更会触发 credential rebuild；client-side reconnect truth 则重新收紧到 `target name + credential handle`

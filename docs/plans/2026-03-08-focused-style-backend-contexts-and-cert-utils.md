# Focused Style Wave: Backend Contexts + Cert Utils (2026-03-08)

## Goal
- 把 `scripts/check_code_style.py src` 中当前 `wolfssl.context`、`mbedtls.context`、`cert.utils` 的 8 处高信号缩进问题收敛到 0。
...
- 新增两个 focused style contracts：
  - `wolfssl.context` + `mbedtls.context`
  - `cert.utils`
- 先用合同脚本制造 RED，再最小修改 8 处 continuation-line 奇数缩进，不改变运行时语义。
...
1. Add:
   ...
   - `tests/scripts/test_focused_style_contract_cert_utils.sh`
2. Run:
   - `bash tests/scripts/test_focused_style_contract_wolfssl_and_mbedtls_contexts.sh`
   - `bash tests/scripts/test_focused_style_contract_cert_utils.sh`
...
- Added:
  ...
  - `tests/scripts/test_focused_style_contract_cert_utils.sh`
- RED runs:
  - `bash tests/scripts/test_focused_style_contract_wolfssl_and_mbedtls_contexts.sh` => FAIL
  - Key failure: `src/fafafa.ssl.wolfssl.context.pas:448 缩进不是 2 空格倍数`
  - `bash tests/scripts/test_focused_style_contract_cert_utils.sh` => FAIL
  - Key failure: `src/fafafa.ssl.cert.utils.pas:512 缩进不是 2 空格倍数`
...
- Added focused contract wiring:
  - `tests/scripts/test_focused_style_contract_wolfssl_and_mbedtls_contexts.sh`
  - `tests/scripts/test_focused_style_contract_cert_utils.sh`
- Reduced the targeted style slice by 8 odd-indentation findings.

### Regression
- `bash tests/scripts/test_focused_style_contract_wolfssl_and_mbedtls_contexts.sh` => PASS
- `bash tests/scripts/test_focused_style_contract_cert_utils.sh` => PASS
...
- `python3 scripts/check_code_style.py src` => FAIL overall, but total errors reduced from `93` to `85` and target files are now clean

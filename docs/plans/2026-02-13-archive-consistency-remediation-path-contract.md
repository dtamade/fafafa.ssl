# Archive Consistency Remediation Path Contract Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** 为 `generate_archive_audit_consistency_remediation_draft.sh` 增加首个 path+strict 合同测试，并修复跨目录调用时相对输入/输出路径不稳定。

**Architecture:** 先新增合同测试复现“在 `/tmp` 调用脚本 + 传仓库相对路径参数”失败，再最小修复输入/输出路径归一化（相对路径回退 `$PROJECT_ROOT`），最后跑 strict 合同与既有脚本合同回归。
...
**Files:**
- Create: `tests/scripts/test_generate_archive_audit_consistency_remediation_path.sh`
- Use fixtures:
  - `docs/test_reports/ARCHIVE_AUDIT_WEEKLY_CHECKLIST_CONSISTENCY_SAMPLE_B33.md`
  - `docs/test_reports/ARCHIVE_AUDIT_BLOCKER_CLOSURE_WAIVER_RECORD_SAMPLE_B36.md`
  - `docs/test_reports/PRE_RELEASE_AUDIT_BLOCKERS_SAMPLE_B32.md`

**Step 1: Write failing test**
- 场景 A：仓库根执行（相对输入 + 相对输出）应生成报告，且包含 `remediation_status=fail`。
- 场景 B：`/tmp` 执行同命令（相对输入 + 相对输出）也应成功；当前预期失败（RED）。

**Step 2: Run RED**
Run: `bash tests/scripts/test_generate_archive_audit_consistency_remediation_path.sh`
...
**Files:**
- Modify: `scripts/generate_archive_audit_consistency_remediation_draft.sh`

**Step 1: Minimal implementation**
- 对三个输入文件参数做归一化：相对路径优先当前目录，否则回退 `$PROJECT_ROOT/<relative>`。
- 对 `--output` 相对路径归一化到 `$PROJECT_ROOT/<relative>`。
- 保持现有 strict 语义：`remediation_status != pass` 时 strict 返回非 0。
...
Run: `bash tests/scripts/test_generate_archive_audit_consistency_remediation_path.sh --strict-check`
Expected: PASS（样例 remediation_status=fail，strict 应返回非 0）。
...
**Step 3: Syntax check**
Run: `bash -n scripts/generate_archive_audit_consistency_remediation_draft.sh`
...
1. **P0（本轮执行）**: `generate_archive_audit_consistency_remediation_draft.sh` path+strict 合同收口。
2. **P1**: `generate_archive_audit_weekly_report_draft.sh` 增加 strict/path 合同测试。
3. **P1**: `check_archive_audit_approval_evidence_consistency_draft.sh` 增加 strict/path 合同测试。

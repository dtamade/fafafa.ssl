...
## Progress Since The GetConnectionInfo Compiler Deprecation Alignment Batch

- 已交付：
  - compiler-level deprecation alignment for `ISSLConnection.GetConnectionInfo`
  - focused source/doc/residual guard:
  ...
  - active docs / v2 doc 已同步：
    - `API_REFERENCE` 直接写明 core getter 当前是编译期 `deprecated`
    - `INTERFACE_DESIGN_V2` 也把 fallback 例子与表格同步成 compiler-deprecated truth
  - residual intentional direct-core tests 已加局部 warning quarantine：
    - backend contract mirror proof
    - WinSSL dedicated core-surface tests

- 当前更准确的 next step：
  - `GetConnectionInfo` 的 source/doc/compiler truth 已基本完成第一条 public slimming slice
  - 默认下一步不该再重复做这条 getter 的 wording/deprecation archaeology
  - 若继续沿同一组 mirrors 推进，应把注意力切到下一条 mirror 的 feasibility / slimming 选择

## Progress Since The GetStateString Compiler Deprecation Alignment Batch

- 已交付：
  - compiler-level deprecation alignment for `ISSLConnection.GetStateString`
  - focused source/doc/residual guard:
  ...
  - active docs / v2 doc 已同步：
    - `API_REFERENCE` 直接写明 core getter 当前是编译期 `deprecated`
    - `INTERFACE_DESIGN_V2` 也把迁移表与 Stage-A note 同步成 compiler-deprecated truth
  - residual direct-core proofs 已加局部 warning quarantine：
    - backend contract mirror proof
    - OpenSSL server OCSP stapling runtime proof
    - WolfSSL server OCSP stapling runtime proof

- 当前更准确的 next step：
  - `GetStateString` 的 source/doc/compiler truth 已完成第一条 public slimming slice
  - 默认下一步不该再重复做这条 getter 的 wording/deprecation archaeology
  - 若继续沿同一组 mirrors 推进，应把注意力切到下一条 mirror 的 feasibility / slimming 选择

## Progress Since The GetSelectedALPNProtocol Compiler Deprecation Alignment Batch

- 已交付：
  - compiler-level deprecation alignment for `ISSLConnection.GetSelectedALPNProtocol`
  - focused source/doc/residual guard:
    - `tests/scripts/test_getselectedalpn_compiler_deprecated_contract.sh`
  - `src/fafafa.ssl.base.pas` 现在明确把 core getter 声明成：
  ...
  - residual direct-core proofs 已加局部 warning quarantine：
    - backend contract mirror proof
    - MbedTLS ALPN runtime proof
    - WinSSL ALPN/SNI runtime proof
    - WinSSL edge-case runtime proof

- 当前更准确的 next step：
  - `GetSelectedALPNProtocol` 的 source/doc/compiler truth 已完成第一条 public slimming slice
  - 默认下一步不该再重复做这条 getter 的 wording/deprecation archaeology

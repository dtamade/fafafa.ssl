# Focused Style Batch: OpenSSL Certificate + OCSP Stapling (2026-03-08)

## Goal
- 把 `scripts/check_code_style.py src` 中当前 `src/fafafa.ssl.openssl.certificate.pas` 与 `src/fafafa.ssl.ocsp.stapling.pas` 的 3 处高信号缩进问题收敛到 0。
...
- 先用合同脚本制造 RED，再最小修改 3 处 continuation-line 奇数缩进，不改变运行时语义。
- 最后将新合同接入 repo hygiene batch，并执行 focused + compile + minimal gate 回归。

## RED
1. Add `tests/scripts/test_focused_style_contract_openssl_certificate_and_ocsp_stapling.sh`
2. Run:
   - `bash tests/scripts/test_focused_style_contract_openssl_certificate_and_ocsp_stapling.sh`
   - Expected: FAIL，提示 `src/fafafa.ssl.openssl.certificate.pas` 仍有 style issue。
...
### RED
- Added `tests/scripts/test_focused_style_contract_openssl_certificate_and_ocsp_stapling.sh`.
- RED run:
  - `bash tests/scripts/test_focused_style_contract_openssl_certificate_and_ocsp_stapling.sh`
  - Result: FAIL
  - Key failure: `src/fafafa.ssl.openssl.certificate.pas:585 缩进不是 2 空格倍数`
...
### Regression
- `bash tests/scripts/test_focused_style_contract_openssl_certificate_and_ocsp_stapling.sh` => PASS
...
- `python3 scripts/check_code_style.py src` => FAIL overall, but total errors reduced from `96` to `93` and target files are now clean

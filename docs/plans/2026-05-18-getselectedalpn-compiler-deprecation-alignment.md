...
把 `ISSLConnection.GetSelectedALPNProtocol` 从“source/doc 已经明确是 compatibility mirror”继续收成真正的源码级 compatibility-only surface：

- `ISSLConnection.GetSelectedALPNProtocol` declaration 进入编译期 `deprecated`
...
1. 给 `ISSLConnection.GetSelectedALPNProtocol` 加编译期 `deprecated` 声明，迁移消息统一导向 `ISSLConnectionInfo.GetSelectedALPNProtocol`。
2. 把 active docs / v2 migration wording 升级成“源码声明已是编译期 deprecated”。
3. 给剩余 direct-core `GetSelectedALPNProtocol` residual proofs 加局部 warning quarantine，保持 compile noise 可控。
4. 新增 focused shell contract，守住：
   - compiler-deprecated declaration
   - docs wording
   - residual intentional tests 的 warning suppression
...
- `GetSelectedALPNProtocol` 在 source/doc/compiler 三层都明确成为 compatibility-only mirror
- 现存 direct-core residual proofs 不会因为新 deprecation 而重新放大 compile noise
- 这条 route 之后不再需要继续做 wording/compiler archaeology，可以开始真正切到下一条 interface-design completeness / implementation-completeness 选择

## Result

- `src/fafafa.ssl.base.pas` 现在把 `ISSLConnection.GetSelectedALPNProtocol` 声明成：
  - `deprecated 'Use ISSLConnectionInfo.GetSelectedALPNProtocol'`
- `API_REFERENCE` 与 `INTERFACE_DESIGN_V2` 现在都明确写出：
  - `GetSelectedALPNProtocol` 在 core 上仅兼容保留
...
- 新增 focused source/doc contract：
  - `tests/scripts/test_getselectedalpn_compiler_deprecated_contract.sh`
- 旧的迁移目标守护脚本也已同步到当前 compiler-deprecated truth：
  - `tests/scripts/test_isslconnectioninfo_migration_targets_contract.sh`

...
清理仓库中 GitHub Actions workflow / template 里残留的 Node20-era JavaScript actions，分四波收口到官方 Node24 默认线：先修 `actions/checkout`，再修 `actions/upload-artifact`，第三波修 `actions/download-artifact`，第四波修 `softprops/action-gh-release`、`actions/setup-python`、`actions/cache`，并保证现有活跃 workflow 与同步模板不回退。
...
10. 复跑关键 workflow contracts，确认 release / signer / completeness 没有回退。
11. 检查同步模板仍保持一致。
12. 更新 working-memory，review，commit 并 push。
13. `download-artifact` 这波只做静态闭环；如未来要补 runtime 证据，单独 dispatch `wave-b-b2-manual.yml`。
14. `setup-lazarus` 继续作为上游阻塞记录，等待官方 Node24 版本。
...
- `.github/workflows` 下不再残留 `actions/checkout@v3/v4`、`actions/upload-artifact@v3/v4/v5`、`actions/download-artifact@v3/v4/v5/v6`、`actions/setup-python@v1-v5`、`actions/cache@v1-v4`，也不再在 release workflow 里保留 `softprops/action-gh-release@v2`

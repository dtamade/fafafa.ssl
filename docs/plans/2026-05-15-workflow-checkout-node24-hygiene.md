# 2026-05-15 Workflow Checkout Node24 Hygiene

## Goal

清理仓库中 GitHub Actions workflow / template 里残留的 `actions/checkout@v3/v4`，把 checkout action 升级到 Node24 兼容线，并保证现有活跃工作流与同步模板不回退。

## Architecture

- GitHub Actions 当前远端 annotation 指向 `actions/checkout@v4`
...
3. 复跑关键 workflow contracts，确认 release / signer / completeness 没有回退。
4. 检查同步模板仍保持一致。
5. 更新 working-memory，review，commit 并 push。
6. 观察新的远端 `CI` / `TLS13 Signer Gate` runs，确认升级后继续通过。
...
- `.github/workflows` 下不再残留 `actions/checkout@v3` 或 `actions/checkout@v4`
- 活跃 workflow 和同步模板显式使用 `actions/checkout@v5`
- release / signer / completeness 合同继续通过
- 远端 CI 不再出现由 checkout 旧 runtime 引发的 Node20 deprecation annotation

...
1. FreePascal early-data 的 default shipped path 已经切到本地持久化 replay-store 路径；默认 ledger 继续复用底层 replay-store seam，不再维持一套独立的单进程内存实现。若默认路径不可用或不可写，resumed early data 会 fail-closed reject。
...
2. capability 等级继续保持 `experimental`；当前剩余 caveat 已经收口到 local persistent 路径、fail-closed 行为，以及尚未承诺 distributed / cross-host replay coordination。

...
所有连接模块现在继承自统一的抽象基类，显著减少代码重复：

| 模块 | 重构前 | 重构后 | 减少 |
|------|--------|--------|------|
| MbedTLS Connection | 705 行 | 566 行 | -139 (-20%) |
| OpenSSL Connection | 1480 行 | 1388 行 | -92 (-6%) |
| WinSSL Connection | 2741 行 | 2169 行 | -572 (-21%) |
| WolfSSL Connection | (内嵌) | 641 行 | 独立模块 |
| Base Class | (无) | 676 行 | 新建 |

**架构设计**：
- 21 个抽象 `Do*` 方法供后端实现底层操作
- 基类提供 ~50 个 ISSLConnection 方法的统一实现

...
┌─────────────────────────────────────┐
│  高层API（最简单）                     │
│  TSSLContextBuilder / TSSLConnector │
│  TSimpleHTTPSClient                  │
│  TCertificateManager                 │
│  TLogger                             │
└──────────────┬──────────────────────┘
...
begin
  ...
  LStream := LTLS.ConnectSocket(YourConnectedSocket, 'example.com');
  // 使用连接...
  LStream.Free;
end;
```
...
for i := 1 to 100 do
begin
  LConnection := LContext.CreateConnection(AConnectedSocket);  // 每轮提供新的已连接 TCP socket
  // 使用连接...
end;

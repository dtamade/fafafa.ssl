...
- **library-scoped defaults**
  - `LogLevel`
  - `LogCallback`
  - 通过 `ISSLLibrary.GetDefaultConfig(...)` / `SetDefaultConfig(...)` 调整 `LogLevel`，通过 `ISSLLibrary.SetLogCallback(...)` 安装回调；`SetDefaultConfig(...)` 不再安装或替换回调；factory request path 不接受 request-local 覆盖。
- **context-scoped**

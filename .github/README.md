...
- 用途：当改动触及 `WinSSL` backend、当前活跃的 WinSSL 示例源码、相关 Windows runtime scripts，或当前会影响 WinSSL runtime proof 的 shared/core units（如 `fafafa.ssl.base` / `fafafa.ssl.connection.base` / `fafafa.ssl.factory` / `fafafa.ssl.context.config` / `fafafa.ssl.asn1` / `fafafa.ssl.x509` / 主门面与 builder）时，自动拉起 Windows runner
- 证据链：
...
- 可选输入 `winssl_enable_native_probe` 可开启有风险的 WinSSL native probe 调查 lane，用来补充 Schannel session evidence；默认保持关闭
